...
namespace ExpressRecipe.NotificationService.Controllers
{
    ...
    public class NotificationController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetNotifications([FromQuery] bool unreadOnly = false, [FromQuery] int limit = 50)
        {
            ...
            List<NotificationDto> notifications = await _repository.GetUserNotificationsAsync(userId, unreadOnly, limit);
        ...
        {
            NotificationDto? notification = await _repository.GetNotificationAsync(id);
        ...
        {
            Guid userId = GetUserId();
            List<NotificationPreferenceDto> prefs = await _repository.GetUserPreferencesAsync(userId);
            return Ok(prefs);
        }
        ...
        {
            Guid userId = GetUserId();
            Guid prefId = await _repository.SavePreferenceAsync(
