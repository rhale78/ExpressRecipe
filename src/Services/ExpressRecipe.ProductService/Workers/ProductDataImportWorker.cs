...
namespace ExpressRecipe.ProductService.Workers
{
    ...
    public class ProductDataImportWorker : BackgroundService
    {
        ...
        private async Task PerformInitialImportIfNeededAsync(CancellationToken stoppingToken)
        {
            ...
            OpenFoodFactsImportService importService = scope.ServiceProvider.GetRequiredService<OpenFoodFactsImportService>();
            ...
            try
            {
                ...
                if (productCount < 1000)
                {
                    ...
                    List<Task<BatchImportResult>> tasks = [];
                    if (csvTask != null)
                    {
                        tasks.Add(csvTask);
                    }
                    ...
                    BatchImportResult[] results = await Task.WhenAll(tasks);
                    ...
                    foreach (BatchImportResult? result in results)
                    {
        ...
        private async Task RunPeriodicDeltaUpdatesAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    BatchImportResult result = await importService.ImportDeltaUpdatesAsync(
                        days: 14,
