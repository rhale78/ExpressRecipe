        ...
        private async Task ProcessStagedProductsAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    IProductStagingRepository stagingRepo = scope.ServiceProvider.GetRequiredService<IProductStagingRepository>();
                    IProductRepository productRepo = scope.ServiceProvider.GetRequiredService<IProductRepository>();
                    IIngredientRepository ingredientRepo = scope.ServiceProvider.GetRequiredService<IIngredientRepository>();
                    IProductImageRepository productImageRepo = scope.ServiceProvider.GetRequiredService<IProductImageRepository>();
                    IIngredientListParser ingredientListParser = scope.ServiceProvider.GetRequiredService<IIngredientListParser>();
                    ILogger<BatchProductProcessor> batchProcessorLogger = scope.ServiceProvider.GetRequiredService<ILogger<BatchProductProcessor>>();
                    ...
                    if (pendingCount > 0)
                    {
                        ...
                        BatchProductProcessor batchProcessor = new BatchProductProcessor(
                        ...
                        ProcessingResult result = await batchProcessor.ProcessStagedProductsAsync(
                            stagingRepo,
                            productRepo,
                            ingredientRepo,
                            productImageRepo,
