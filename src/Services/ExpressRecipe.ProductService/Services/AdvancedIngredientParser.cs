...
namespace ExpressRecipe.ProductService.Services
{
    ...
    public class AdvancedIngredientParser : IIngredientListParser
    {
        ...
        public List<string> ParseIngredients(string ingredientsText)
        {
            ...
            HashSet<string> ingredients = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            ...
            // Clean and filter
            List<string> cleaned = ingredients
                ...
                .Where(ingredient =>
                {
                    ...
                    IngredientValidationResult validation = ValidateIngredient(ingredient);
        ...
        private void ExtractIngredients(string text, HashSet<string> ingredients)
        {
            ...
            Match containsMatch = ContainsPatternRegex.Match(text);
        ...
        private void ExtractFromSegment(string segment, HashSet<string> ingredients)
        {
            ...
            List<string> parts = SplitRespectingParentheses(segment, ',');
            ...
            foreach (var part in parts)
            {
                ...
                Match parenMatch = Regex.Match(trimmed, @"^([^(]+)\s*\(([^)]+)\)(.*)$");
                ...
                if (parenMatch.Success)
                {
                    ...
                    List<string> subParts = SplitRespectingParentheses(subIngredients, ',');
        ...
        private string CleanIngredientName(string ingredient)
        {
            ...
            ingredient = Regex.Replace(ingredient, @"\s*\([^)]*\)\s*", match =>
            {
                ...
                return content.Contains(',') || Regex.IsMatch(content, @"\b(milk|salt|sugar|water|oil)\b", RegexOptions.IgnoreCase)
                    ? match.Value
                    : " ";
            }).Trim();
        ...
        private bool IsValidIngredient(string ingredient)
        {
            ...
            return specialCharCount <= ingredient.Length / 3;
        }
    }
}
