    ...
    public class OpenFoodFactsImportService
    {
        ...
        public async Task<ImportResult> ImportProductByBarcodeAsync(string barcode)
        {
            try
            {
                ...
                HttpResponseMessage response = await _httpClient.GetAsync($"api/v2/product/{barcode}.json");
                ...
                ImportResult result = await ProcessProductAsync(product, barcode);
        ...
        private void LogImageFieldsFromJson(JsonElement product, string barcode)
        {
            try
            {
                ...
                if (product.TryGetProperty("selected_images", out JsonElement selectedImages))
                {
        ...
        {
            if (selectedImages.TryGetProperty(imageType, out JsonElement imageTypeElement))
            {
                _logger.LogInformation("    {ImageType} image found in selected_images", imageType);

                if (imageTypeElement.TryGetProperty("display", out JsonElement display))
                {
                    _logger.LogInformation("      display property found with keys: {Keys}",
                        string.Join(", ", display.EnumerateObject().Select(p => p.Name)));

                    foreach (JsonProperty lang in display.EnumerateObject())
                    {
        ...
        public async Task<BatchImportResult> SearchAndImportAsync(string query, int pageSize = 20, int maxResults = 100)
        {
            ...
            try
            {
                ...
                if (searchResult.RootElement.TryGetProperty("products", out JsonElement products))
                {
                    int count = 0;
                    foreach (JsonElement product in products.EnumerateArray())
                    {
                        ...
                        try
                        {
                            var barcode = product.TryGetProperty("code", out JsonElement bc) ? bc.GetString() : null;
                            if (!string.IsNullOrEmpty(barcode))
                            {
                                ...
                                ImportResult importResult = await ProcessProductAsync(product, barcode);
        ...
        public async Task<BatchImportResult> ImportByCategoryAsync(string category, int limit = 100)
        {
            ...
            try
            {
                ...
                if (data.RootElement.TryGetProperty("products", out JsonElement products))
                {
                    foreach (JsonElement product in products.EnumerateArray())
                    {
                        try
        ...
        private async Task<ImportResult> ProcessProductAsync(JsonElement product, string barcode)
        {
            ...
            try
            {
                ...
                ProductDto? existing = await _productRepository.GetProductByBarcodeAsync(barcode);
                ...
                // Create product
                Guid productId = await _productRepository.CreateProductAsync(new Shared.DTOs.Product.CreateProductRequest {
                ...
                {
                    List<string> ingredients = _ingredientListParser.ParseIngredients(ingredientsText);
                    int orderIndex = 0;
                ...
                if (product.TryGetProperty("nutriments", out JsonElement nutriments))
                {
                ...
                {
                    List<string> allergenList = allergens.Split(',').Select(a => a.Trim()).ToList();
        ...
        private async Task LinkToExternalDatabasesAsync(Guid productId, JsonElement product)
        {
            try
            {
                ...
                if (product.TryGetProperty("nova_group", out JsonElement novaGroup) && novaGroup.ValueKind == JsonValueKind.Number)
                {
        ...
        {
            Dictionary<string, decimal?> nutritionData = new Dictionary<string, decimal?>
            {
                // Extract per 100g values (OpenFoodFacts standard)
                ["Calories"] = GetDecimalValue(nutriments, "energy-kcal_100g", "energy_100g"),
                ["Protein"] = GetDecimalValue(nutriments, "proteins_100g"),
                ["TotalFat"] = GetDecimalValue(nutriments, "fat_100g"),
                ["SaturatedFat"] = GetDecimalValue(nutriments, "saturated-fat_100g"),
                ["Carbohydrates"] = GetDecimalValue(nutriments, "carbohydrates_100g"),
                ["Sugars"] = GetDecimalValue(nutriments, "sugars_100g"),
                ["Fiber"] = GetDecimalValue(nutriments, "fiber_100g"),
                ["Sodium"] = GetDecimalValue(nutriments, "sodium_100g"),
                ["Salt"] = GetDecimalValue(nutriments, "salt_100g")
            };
        ...
        private string? DetermineCategory(string? categories, string? productName)
        {
            ...
            return categoryText.Contains("beverage") || categoryText.Contains("drink") || categoryText.Contains("juice")
                ? "Beverages"
                : categoryText.Contains("snack") || categoryText.Contains("dessert") || categoryText.Contains("candy") || categoryText.Contains("chocolate")
                ? "Snacks & Sweets"
                : categoryText.Contains("sauce") || categoryText.Contains("condiment") || categoryText.Contains("spice")
                ? "Condiments & Sauces"
                : "General";
        }
        ...
        private string? GetStringValue(JsonElement element, params string[] fieldNames)
        {
            ...
            {
                if (element.TryGetProperty(fieldName, out JsonElement value) && value.ValueKind == JsonValueKind.String)
                {
        ...
        private string? GetNestedImageUrl(JsonElement product, string imageType = "front", string sizeKey = "display")
        {
            try
            {
                ...
                if (product.TryGetProperty("selected_images", out JsonElement selectedImages))
                {
                    if (selectedImages.TryGetProperty(imageType, out JsonElement imageTypeElement))
                    {
                        if (imageTypeElement.TryGetProperty(sizeKey, out JsonElement sizeElement))
                        {
                            // Try language-specific first
                            var lang = GetStringValue(product, "lang", "lc") ?? "en";
                            if (sizeElement.TryGetProperty(lang, out JsonElement langImage))
                            {
                            ...
                            // Try "en" as fallback
                            if (sizeElement.TryGetProperty("en", out JsonElement enImage))
                            {
                            ...
                            foreach (JsonProperty prop in sizeElement.EnumerateObject())
                            {
        ...
        private decimal? GetDecimalValue(JsonElement element, params string[] fieldNames)
        {
            foreach (var fieldName in fieldNames)
            {
                if (element.TryGetProperty(fieldName, out JsonElement value))
                {
            ...
            IProgress<ImportProgress>? progress = null)
        {
            ...
            try
            {
                ...
                using HttpResponseMessage response = await _httpClient.GetAsync(dataFileUrl, HttpCompletionOption.ResponseHeadersRead, cancellationToken);
                response.EnsureSuccessStatusCode();

                using Stream stream = await response.Content.ReadAsStreamAsync(cancellationToken);
                using GZipStream gzipStream = new System.IO.Compression.GZipStream(stream, System.IO.Compression.CompressionMode.Decompress);
                ...
                int skippedCount = 0;
                var batchSize = 5000;
                List<StagedProduct> stagingBatch = [];
                ...
                while (!reader.EndOfStream && processedCount < maxProducts && !cancellationToken.IsCancellationRequested)
                {
                    ...
                    try
                    {
                        JsonElement product = JsonDocument.Parse(line).RootElement;
                        ...
                        StagedProduct stagedProduct = MapToStagedProduct(product, barcode);
                        ...
                        if (stagingBatch.Count >= batchSize)
                        {
                            ...
                            if (enableAugmentation)
                            {
                                ...
                                {
                                    List<StagedProduct> withImages = stagingBatch.Where(p => !string.IsNullOrWhiteSpace(p.ImageUrl)).ToList();
                                    if (withImages.Count != 0)
                                    {
                                        ...
                                        foreach (StagedProduct? p in withImages.Take(3))
                                        {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                List<string> deltaFiles = indexContent.Split('\n', StringSplitOptions.RemoveEmptyEntries)
                ...
                foreach (var deltaFile in deltaFiles)
                {
                    ...
                    BatchImportResult deltaResult = await ImportFromBulkDataAsync(deltaUrl, remainingQuota, cancellationToken);
            ...
            BatchImportResult result = new BatchImportResult();
                ...
                List<StagedProduct> stagingBatch = [];
                    ...
                    while (await csv.ReadAsync() && processedCount < maxProducts && !cancellationToken.IsCancellationRequested)
                    {
                        ...
                        StagedProduct stagedProduct = new StagedProduct
                            {
                                ExternalId = barcode,
                                Barcode = barcode,
        ...
        private StagedProduct MapToStagedProduct(JsonElement product, string barcode)
        {
            ...
            if (product.TryGetProperty("allergens_hierarchy", out JsonElement allergensHierarchy))
            {
            ...
            if (product.TryGetProperty("categories_hierarchy", out JsonElement categoriesHierarchy))
            {
