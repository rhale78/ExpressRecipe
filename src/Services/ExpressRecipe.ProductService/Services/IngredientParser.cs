...
namespace ExpressRecipe.ProductService.Services
{
    ...
    public class IngredientParser : IIngredientParser
    {
        ...
        public async Task<ParsedIngredientResult> ParseIngredientStringAsync(string ingredientString)
        {
            ...
            ParsedIngredientResult result = new ParsedIngredientResult
            {
                OriginalString = ingredientString
            };

            // Split by commas at the top level (not inside parentheses)
            List<string> topLevelComponents = SplitRespectingParentheses(ingredientString, ',');

            for (int i = 0; i < topLevelComponents.Count; i++)
            {
                ParsedIngredientComponent? component = ParseComponent(topLevelComponents[i].Trim(), i);
                if (component != null)
                {
        ...
        {
            ParsedIngredientResult parsedResult = await ParseIngredientStringAsync(ingredientString);
            List<Guid> linkedIds = [];

            foreach (ParsedIngredientComponent component in parsedResult.Components)
            {
                ...
                {
                    foreach (ParsedIngredientComponent subComponent in component.SubComponents)
                    {
        ...
        private ParsedIngredientComponent? ParseComponent(string componentString, int orderIndex)
        {
            ...
            ParsedIngredientComponent component = new ParsedIngredientComponent
            {
            ...
            Match match = ParenthesesRegex.Match(componentString);
            ...
            if (match.Success)
            {
                ...
                List<string> subComponentParts = SplitRespectingParentheses(subComponentsString, ',');

                component.SubComponents = [];
                for (int i = 0; i < subComponentParts.Count; i++)
                {
                    ParsedIngredientComponent subComponent = new ParsedIngredientComponent
                    {
        ...
        private async Task MatchComponentsToBaseIngredientsAsync(List<ParsedIngredientComponent> components)
        {
            foreach (ParsedIngredientComponent component in components)
            {
                ...
                BaseIngredientDto? baseIngredient = await _baseIngredientRepository.FindByNameAsync(component.Name);
                ...
                else
                {
                    ...
                    List<BaseIngredientDto> searchResults = await _baseIngredientRepository.SearchAsync(new BaseIngredientSearchRequest
                    {
                    ...
                    {
                        BaseIngredientDto match = searchResults.First();
