...
namespace ExpressRecipe.ProductService.Services
{
    ...
    public class BatchProductProcessor
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            ProgressTracker progressTracker = new ProgressTracker(_logger, pendingCount);
            ...
            TransformManyBlock<int, StagedProduct> fetchBlock = new TransformManyBlock<int, StagedProduct>(
                async batchNumber =>
                {
                    try
                    {
                        List<StagedProduct> products = await stagingRepo.GetPendingProductsAsync(_batchSize);
            ...
            BatchBlock<StagedProduct> batchBlock = new BatchBlock<StagedProduct>(_batchSize, new GroupingDataflowBlockOptions
            {
                BoundedCapacity = _bufferSize
            });

            // Stage 3: Pre-create unique ingredients for each batch
            TransformBlock<StagedProduct[], StagedProduct[]> preCreateIngredientsBlock = new TransformBlock<StagedProduct[], StagedProduct[]>(
                async batch =>
                {
                    try
                    {
            ...
            ActionBlock<StagedProduct[]> processBatchBlock = new ActionBlock<StagedProduct[]>(
                async batch =>
                {
                    try
                    {
                        (int Success, int Failure) batchResult = await ProcessProductBatchAsync(batch, productRepo, ingredientRepo, productImageRepo, stagingRepo, cancellationToken);
                    ...
                    catch (Exception ex)
                    {
                        ...
                        // Mark all as failed
                        foreach (StagedProduct product in batch)
                        {
                            try
                            {
            ...
            CancellationToken cancellationToken)
        {
            ...
            Dictionary<string, Guid> batchIngredientCache = await PreFetchBatchIngredientsAsync(batch, ingredientRepo, cancellationToken);

            // Step 1: Check which barcodes already exist (batch lookup)
            List<string> barcodes = batch
                .Where(p => !string.IsNullOrWhiteSpace(p.Barcode))
                .Select(p => p.Barcode!)
                .Distinct()
                .ToList();

            HashSet<string> existingProducts = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            if (barcodes.Count != 0)
            {
                IEnumerable<string> existingBarcodes = await productRepo.GetExistingBarcodesAsync(barcodes);
            ...
            List<ProductWithId> newProducts = batch
            ...
            if (newProducts.Count == 0)
            {
                ...
                List<Guid> existingIds = batch.Select(p => p.Id).ToList();
            ...
            List<Shared.DTOs.Product.CreateProductRequest> productRequests = newProducts.Select(p => p.Request).ToList();
            ...
            List<string> createdBarcodes = newProducts
            ...
            List<(Guid ProductId, Guid IngredientId, int OrderIndex)> ingredientLinks = [];
            foreach (ProductWithId product in newProducts)
            {
                ...
                {
                    List<string> parsedIngredients = ParseIngredientsQuick(ingredientsText).Take(50).ToList();

                    // PERFORMANCE: Use cached batch ingredients instead of another lookup
                    int orderIndex = 0;
                    foreach (var ingredientName in parsedIngredients)
                    {
                        if (batchIngredientCache.TryGetValue(ingredientName, out Guid ingredientId))
                        {
            ...
            List<(Guid ProductId, string Source, string ExternalId)> externalLinks = newProducts
            ...
            List<Guid> completedIds = newProducts.Select(p => p.StagedProduct.Id).ToList();
            List<Guid> skippedIds = batch
                .Where(p => !string.IsNullOrWhiteSpace(p.Barcode) && existingProducts.Contains(p.Barcode!))
                .Select(p => p.Id)
                .ToList();
            ...
            CancellationToken cancellationToken)
        {
            ...
            HashSet<string> allIngredientNames = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            ...
            foreach (StagedProduct product in batch)
            {
                ...
                if (!string.IsNullOrWhiteSpace(ingredientsText))
                {
                    List<string> ingredients = ParseIngredientsQuick(ingredientsText).Take(50).ToList();
            ...
            Dictionary<string, Guid> ingredientIds = await ingredientRepo.GetIngredientIdsByNamesAsync(allIngredientNames);
            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (StagedProduct product in stagedProducts)
            {
                var ingredientsText = product.IngredientsTextEn ?? product.IngredientsText;
                if (!string.IsNullOrWhiteSpace(ingredientsText))
                {
                    List<string> ingredients = ParseIngredientsQuick(ingredientsText);
                    foreach (var ingredient in ingredients)
                    {
                        if (!string.IsNullOrWhiteSpace(ingredient))
                        {
            ...
            Dictionary<string, Guid> existingIngredients = await ingredientRepo.GetIngredientIdsByNamesAsync(allIngredientNames);

            // Create new ingredients in bulk
            List<string> newIngredients = allIngredientNames
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                ProductDto? existing = await productRepo.GetByBarcodeAsync(stagedProduct.Barcode);
                if (existing != null)
                {
            ...
            // Create product
            Shared.DTOs.Product.CreateProductRequest productRequest = new Shared.DTOs.Product.CreateProductRequest
            {
            ...
            Guid productId = await productRepo.CreateAsync(productRequest);
            ...
            if (!string.IsNullOrWhiteSpace(ingredientsText))
            {
                List<string> parsedIngredients = ParseIngredientsQuick(ingredientsText).Take(50).ToList(); // Limit to 50 ingredients

                // PERFORMANCE FIX: Bulk lookup ingredient IDs instead of N queries
                Dictionary<string, Guid> ingredientIds = await ingredientRepo.GetIngredientIdsByNamesAsync(parsedIngredients);
                ...
                foreach (var ingredientName in parsedIngredients)
                {
                    ...
                    if (ingredientIds.TryGetValue(ingredientName, out Guid ingredientId))
                    {
                        try
                        {
                            AddProductIngredientRequest addIngredientRequest = new AddProductIngredientRequest
                            {
