...
public class HighSpeedProductRepository : SqlHelper, IHighSpeedProductRepository
{
        ...
        CancellationToken ct = default)
    {
        ...
        {
            // Create DataTable
            var dataTable = CreateProductDataTable();
        ...
        CancellationToken ct = default)
    {
        ...
        var dataTable = CreateProductUpdateDataTable();
        foreach (var product in productsList)
        {
            var row = dataTable.NewRow();
        ...
        CancellationToken ct = default)
    {
        ...
        var result = await ExecuteBatchLookupAsync<Guid, ProductDto>(
            "Product",
            "Id",
            idsList,
        ...
        CancellationToken ct = default)
    {
        ...
        if (!barcodesList.Any()) return new Dictionary<string, ProductDto>();
        ...
        // Create temp table
        await using (var createCmd = new SqlCommand($"CREATE TABLE {tempTableName} ([Barcode] NVARCHAR(50) PRIMARY KEY)", connection))
        {
        ...
        await using (var queryCmd = new SqlCommand(querySql, connection))
        {
            ...
            {
                var product = MapReaderToProductDto(reader);
        ...
        CancellationToken ct = default)
    {
        ...
        if (uncachedIds.Any())
        {
            var dbProducts = await GetByIdsAsync(uncachedIds, ct);
