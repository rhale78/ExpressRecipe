...
namespace ExpressRecipe.ProductService.Data
{
    ...
    public class AllergenRepositoryAdapter : IAllergenRepository
    {
        ...
        public async Task<List<ProductDto>> FindProductsWithoutAllergensAsync(List<string> allergens, int limit = 100)
        {
            ...
            await using SqlConnection conn = new SqlConnection(_dbConnection.ConnectionString);
        ...
        public async Task<bool> ProductContainsAllergensAsync(Guid productId, List<string> allergens)
        {
            ...
            List<SqlParameter> parameters = [new SqlParameter("@ProductId", System.Data.SqlDbType.UniqueIdentifier) { Value = productId }];
            ...
            _ = Task.Run(async () =>
            {
                try
                {
                    foreach (var allergen in allergens)
                    {
                        ...
                        await using SqlConnection conn = new SqlConnection(_dbConnection.ConnectionString);
                        await conn.OpenAsync();
                        await using SqlCommand cmd = new SqlCommand(mergeSql, conn);
        ...
        public Task<List<ProductDto>> FindProductsByDietaryRestrictionAsync(string restriction, int limit = 100)
        {
            ...
            switch (restrictionLower)
            {
                ...
                case "gluten-free":
                case "gluten free":
                    allergenRestrictions = ["gluten", "wheat", "barley", "rye", "malt", "spelt", "kamut"];
                    break;
                case "dairy-free":
                case "dairy free":
                case "lactose-free":
                case "lactose free":
                    allergenRestrictions = ["milk", "dairy", "whey", "casein", "lactose", "butter", "cream", "cheese", "yogurt"];
                    break;
                case "nut-free":
                case "nut free":
                    allergenRestrictions = ["peanut", "almond", "cashew", "walnut", "pecan", "hazelnut", "pistachio", "macadamia", "pine nut", "brazil nut"];
                    break;
                case "soy-free":
                case "soy free":
                    allergenRestrictions = ["soy", "soya", "tofu", "edamame", "tempeh", "miso"];
                    break;
                default:
                    return Task.FromResult(new List<ProductDto>());
            }
