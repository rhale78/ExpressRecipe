...
namespace ExpressRecipe.ProductService.Controllers
{
    ...
    public class IngredientsController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<IngredientDto>>> GetAll()
        {
            try
            {
                List<IngredientDto> ingredients = await _repository.GetAllAsync();
        ...
        public async Task<ActionResult<IngredientDto>> GetById(Guid id)
        {
            try
            {
                IngredientDto? ingredient = await _repository.GetByIdAsync(id);

                return ingredient == null ? (ActionResult<IngredientDto>)NotFound(new { message = "Ingredient not found" }) : (ActionResult<IngredientDto>)Ok(ingredient);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<IngredientDto>>> Search([FromQuery] string q)
        {
            try
            {
                ...
                List<IngredientDto> ingredients = await _repository.SearchByNameAsync(q);
                return Ok(ingredients);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<IngredientDto>>> GetByCategory(string category)
        {
            try
            {
                List<IngredientDto> ingredients = await _repository.GetByCategoryAsync(category);
                return Ok(ingredients);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<IngredientDto>> Create([FromBody] CreateIngredientRequest request)
        {
            try
            {
                ...
                IngredientDto? ingredient = await _repository.GetByIdAsync(ingredientId);
