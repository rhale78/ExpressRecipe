...
namespace ExpressRecipe.ProductService.Controllers
{
    ...
    public class RestaurantsController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<RestaurantDto>>> Search([FromQuery] RestaurantSearchRequest request)
        {
            try
            {
                List<RestaurantDto> restaurants = await _repository.SearchAsync(request);
        ...
        public async Task<ActionResult<RestaurantDto>> GetById(Guid id)
        {
            try
            {
                ...
                return restaurant == null ? (ActionResult<RestaurantDto>)NotFound(new { message = "Restaurant not found" }) : (ActionResult<RestaurantDto>)Ok(restaurant);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<RestaurantDto>> Create([FromBody] CreateRestaurantRequest request)
        {
            try
            {
                ...
                Guid restaurantId = await _repository.CreateAsync(request, userId.Value);
                RestaurantDto? restaurant = await _repository.GetByIdAsync(restaurantId);
        ...
        public async Task<ActionResult<List<UserRestaurantRatingDto>>> GetRatings(Guid id)
        {
            try
            {
                List<UserRestaurantRatingDto> ratings = await _repository.GetRestaurantRatingsAsync(id);
                return Ok(ratings);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<UserRestaurantRatingDto>> GetMyRating(Guid id)
        {
            try
            {
                ...
                UserRestaurantRatingDto? rating = await _repository.GetUserRatingAsync(id, userId.Value);

                return rating == null ? (ActionResult<UserRestaurantRatingDto>)NotFound(new { message = "Rating not found" }) : (ActionResult<UserRestaurantRatingDto>)Ok(rating);
            }
            catch (Exception ex)
            {
