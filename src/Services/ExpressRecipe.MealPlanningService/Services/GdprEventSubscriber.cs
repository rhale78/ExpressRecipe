...
/// <summary>
/// Subscribes to <see cref="GdprDeleteEvent"/> and hard-deletes all meal planning data
/// owned by the user (meal plans, planned meals, cooking history).
/// </summary>
public sealed class GdprEventSubscriber : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("[MealPlanningService] Subscribed to GDPR delete events");
    }
    ...
    {
        _logger.LogWarning(
            "[MealPlanningService] GDPR delete received for user {UserId} (requestId {RequestId})",
            evt.UserId, evt.RequestId);

        try
        {
            using IServiceScope scope = _scopeFactory.CreateScope();
            IMealPlanningRepository repo = scope.ServiceProvider.GetRequiredService<IMealPlanningRepository>();
            await repo.DeleteUserDataAsync(evt.UserId, cancellationToken);
            _logger.LogInformation(
                "[MealPlanningService] GDPR delete complete for user {UserId}", evt.UserId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex,
                "[MealPlanningService] Error processing GDPR delete for user {UserId}", evt.UserId);
        }
    }
}
