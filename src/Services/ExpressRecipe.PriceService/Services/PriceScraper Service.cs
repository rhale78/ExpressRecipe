    ...
    public class PriceScraperService
    {
        ...
        public async Task<PriceData?> ScrapeWalmartAsync(string productUrl)
        {
            try
            {
                ...
                HtmlNode priceNode = doc.DocumentNode.SelectSingleNode("//span[@itemprop='price']")
                ...
                HtmlNode nameNode = doc.DocumentNode.SelectSingleNode("//h1[@itemprop='name']")
        ...
        public async Task<PriceData?> ScrapeTargetAsync(string productUrl)
        {
            try
            {
                ...
                HtmlNode priceNode = doc.DocumentNode.SelectSingleNode("//span[@data-test='product-price']")
                ...
                // Extract product name
                HtmlNode nameNode = doc.DocumentNode.SelectSingleNode("//h1[@data-test='product-title']")
                            ?? doc.DocumentNode.SelectSingleNode("//h1");
                var productName = nameNode?.InnerText.Trim();
        ...
        public async Task<PriceData?> ScrapeAmazonAsync(string productUrl)
        {
            try
            {
                ...
                HtmlNode priceNode = doc.DocumentNode.SelectSingleNode("//span[@class='a-price-whole']")
                ...
                // Extract product name
                HtmlNode nameNode = doc.DocumentNode.SelectSingleNode("//span[@id='productTitle']")
                            ?? doc.DocumentNode.SelectSingleNode("//h1[@id='title']");
                var productName = nameNode?.InnerText.Trim();

                // Check availability
                HtmlNode availNode = doc.DocumentNode.SelectSingleNode("//div[@id='availability']");
        ...
        public async Task<PriceData?> ScrapeKrogerAsync(string productUrl)
        {
            try
            {
                ...
                HtmlNode priceNode = doc.DocumentNode.SelectSingleNode("//div[contains(@class, 'ProductDetails-sellBy-price')]")
                ...
                // Extract product name
                HtmlNode nameNode = doc.DocumentNode.SelectSingleNode("//h1[contains(@class, 'ProductDetails')]")
                            ?? doc.DocumentNode.SelectSingleNode("//h1");
                var productName = nameNode?.InnerText.Trim();
        ...
        private decimal? ExtractPrice(string priceText)
        {
            try
            {
                ...
                return string.IsNullOrWhiteSpace(cleanText) ? null : decimal.TryParse(cleanText, out var price) ? price : null;
            }
            catch
            {
                return null;
