...
namespace ExpressRecipe.PriceService.Controllers
{
    ...
    public class PriceController : ControllerBase
    {
        ...
        {
            List<StoreDto> stores = await _repository.GetStoresAsync(city, state, chain);
            return Ok(stores);
        }
        ...
        {
            Guid storeId = await _repository.AddStoreAsync(
        ...
        public async Task<IActionResult> RecordPrice([FromBody] RecordPriceRequest request)
        {
            ...
            Guid priceId = await _repository.RecordPriceAsync(
        ...
        {
            List<PriceObservationDto> prices = await _repository.GetProductPricesAsync(productId, storeId, daysBack);
            return Ok(prices);
        }
        ...
        {
            PriceTrendDto trend = await _repository.GetPriceTrendAsync(productId, storeId);
            return Ok(trend);
        }
        ...
        {
            List<DealDto> deals = await _repository.GetActiveDealsAsync(storeId, productId);
            return Ok(deals);
        }
        ...
        {
            Guid dealId = await _repository.CreateDealAsync(
        ...
        {
            List<StorePriceComparisonDto> comparison = await _repository.ComparePricesAsync(request.ProductIds, request.StoreIds);
