    ...
    public class RecipeImportService
    {
        ...
        public async Task<ImportJobResult> ProcessImportJobAsync(Guid jobId, Guid userId, string content, ParserContext context)
        {
            ...
            try
            {
                ...
                RecipeImportJobDto? job = await _importRepository.GetImportJobByIdAsync(jobId);
                if (job == null)
                {
                ...
                RecipeImportSourceDto? source = await _importRepository.GetImportSourceByIdAsync(job.ImportSourceId);
                if (source == null)
                {
                ...
                IRecipeParser? parser = string.IsNullOrEmpty(source.ParserClassName)
                ...
                foreach (ParsedRecipe parsedRecipe in parsedRecipes)
                {
                    try
                    {
                        // Check for duplicates
                        RecipeDto? duplicate = await _recipeRepository.FindDuplicateRecipeAsync(parsedRecipe.Name, userId);
                        ...
                        CreateRecipeRequest createRequest = ConvertToCreateRequest(parsedRecipe, userId, imageUrl);

                        // Create the recipe
                        Guid recipeId = await _recipeRepository.CreateRecipeAsync(createRequest, userId);

                        _logger.LogInformation("Created recipe {RecipeId}: {RecipeName}", recipeId, parsedRecipe.Name);

                        // Add ingredients
                        if (parsedRecipe.Ingredients.Count != 0)
                        {
                            List<RecipeIngredientDto> ingredients = parsedRecipe.Ingredients.Select(i => new RecipeIngredientDto
                            {
                        ...
                        if (parsedRecipe.Instructions.Count != 0)
                        {
                        ...
                        RecipeNutritionDto? nutrition = _nutritionService.ExtractNutrition(parsedRecipe);
                        if (nutrition != null)
                        {
                        ...
                        List<RecipeAllergenWarningDto> allergens = await _allergenService.DetectAllergensAsync(parsedRecipe);
        ...
        {
            ValidationResult result = new ValidationResult { IsValid = true, Errors = [] };

            try
            {
                // Get the import source
                RecipeImportSourceDto? source = await _importRepository.GetImportSourceByIdAsync(importSourceId);
                if (source == null)
                {
                ...
                List<ParsedRecipe> recipes = await parser.ParseAsync(content, context);
