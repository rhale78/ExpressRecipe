...
namespace ExpressRecipe.RecipeService.Services
{
    ...
    public class AllergenDetectionService
    {
        ...
        private static readonly Dictionary<string, List<string>> AllergenKeywords = new()
        {
            ...
            ["Fish"] = ["fish", "salmon", "tuna", "cod", "halibut", "anchovy", "anchovies"],
            ["Shellfish"] = ["shrimp", "crab", "lobster", "shellfish", "clam", "oyster", "mussel", "scallop"],
            ["Tree Nuts"] = ["almond", "walnut", "pecan", "cashew", "pistachio", "hazelnut", "macadamia", "nut"],
            ["Peanuts"] = ["peanut", "peanuts", "peanut butter"],
            ["Wheat"] = ["wheat", "flour", "bread", "pasta", "noodle", "semolina", "farina", "bulgur", "couscous"],
            ["Soybeans"] = ["soy", "tofu", "tempeh", "edamame", "miso", "soy sauce", "tamari"],
            ["Sesame"] = ["sesame", "tahini"],
            ["Gluten"] = ["wheat", "barley", "rye", "flour", "bread", "pasta", "seitan", "malt"]
        };
        ...
        {
            List<RecipeAllergenWarningDto> allergenWarnings = [];
            HashSet<Guid> detectedAllergens = []; // Track to avoid duplicates

            foreach (ParsedIngredient ingredient in recipe.Ingredients)
            {
                // Try database lookup first
                List<(Guid AllergenId, string AllergenName)> dbAllergens = await FindAllergensInDatabaseAsync(ingredient.IngredientName);

                foreach ((Guid AllergenId, string AllergenName) allergen in dbAllergens)
                {
                ...
                if (dbAllergens.Count == 0)
                {
                    List<(string AllergenName, string Keyword)> keywordAllergens = DetectByKeywords(ingredient.IngredientName);
                    foreach ((string? allergenName, string _) in keywordAllergens)
                    {
                        // Create a pseudo-ID for keyword-based allergens
                        // In production, these should be matched to actual allergen IDs
                        Guid pseudoId = GenerateAllergenId(allergenName);
        ...
        {
            List<(string, string)> detected = [];
            var lowerIngredient = ingredientName.ToLower();

            foreach ((string? allergenName, List<string>? keywords) in AllergenKeywords)
            {
