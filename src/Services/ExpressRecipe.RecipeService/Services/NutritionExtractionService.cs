...
namespace ExpressRecipe.RecipeService.Services
{
    ...
    public class NutritionExtractionService
    {
        ...
        private bool ExtractFromNutritionField(string nutritionText, RecipeNutritionDto nutrition)
        {
            ...
            // Calories
            Match caloriesMatch = Regex.Match(nutritionText, @"(\d+(?:\.\d+)?)\s*(?:cal|kcal|calories)", RegexOptions.IgnoreCase);
            ...
            // Fat
            Match fatMatch = Regex.Match(nutritionText, @"(?:total\s+)?fat[:\s]+(\d+(?:\.\d+)?)\s*g", RegexOptions.IgnoreCase);
            ...
            // Saturated Fat
            Match satFatMatch = Regex.Match(nutritionText, @"saturated\s+fat[:\s]+(\d+(?:\.\d+)?)\s*g", RegexOptions.IgnoreCase);
            ...
            // Cholesterol
            Match cholMatch = Regex.Match(nutritionText, @"cholesterol[:\s]+(\d+(?:\.\d+)?)\s*mg", RegexOptions.IgnoreCase);
            ...
            // Sodium
            Match sodiumMatch = Regex.Match(nutritionText, @"sodium[:\s]+(\d+(?:\.\d+)?)\s*mg", RegexOptions.IgnoreCase);
            ...
            // Carbohydrates
            Match carbMatch = Regex.Match(nutritionText, @"(?:total\s+)?carb(?:ohydrate)?s?[:\s]+(\d+(?:\.\d+)?)\s*g", RegexOptions.IgnoreCase);
            ...
            // Fiber
            Match fiberMatch = Regex.Match(nutritionText, @"(?:dietary\s+)?fiber[:\s]+(\d+(?:\.\d+)?)\s*g", RegexOptions.IgnoreCase);
            ...
            // Sugar
            Match sugarMatch = Regex.Match(nutritionText, @"(?:total\s+)?sugar(?:s)?[:\s]+(\d+(?:\.\d+)?)\s*g", RegexOptions.IgnoreCase);
            ...
            // Protein
            Match proteinMatch = Regex.Match(nutritionText, @"protein[:\s]+(\d+(?:\.\d+)?)\s*g", RegexOptions.IgnoreCase);
        ...
        private bool ExtractFromText(string text, RecipeNutritionDto nutrition)
        {
            ...
            Match perServingMatch = Regex.Match(text,
