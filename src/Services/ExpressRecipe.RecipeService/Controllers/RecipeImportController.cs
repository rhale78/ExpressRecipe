    ...
    public class RecipeImportController : ControllerBase
    {
        ...
        public async Task<ActionResult<ImportSummaryDto>> GetSummary()
        {
            try
            {
                ...
                ImportSummaryDto summary = await _importRepository.GetImportSummaryAsync(userId.Value);
                return Ok(summary);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<RecipeImportSourceDto>>> GetImportSources([FromQuery] bool activeOnly = true)
        {
            try
            {
                List<RecipeImportSourceDto> sources = await _importRepository.GetImportSourcesAsync(activeOnly);
                return Ok(sources);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<RecipeImportJobDto>>> GetJobs([FromQuery] int limit = 50)
        {
            try
            {
                ...
                List<RecipeImportJobDto> jobs = await _importRepository.GetUserImportJobsAsync(userId.Value, limit);
                return Ok(jobs);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<RecipeImportJobDto>> GetJob(Guid id)
        {
            try
            {
                RecipeImportJobDto? job = await _importRepository.GetImportJobByIdAsync(id);
                ...
                // Verify ownership
                Guid? userId = GetCurrentUserId();
                return job.UserId != userId ? (ActionResult<RecipeImportJobDto>)Forbid() : (ActionResult<RecipeImportJobDto>)Ok(job);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<Guid>> StartImportJob([FromBody] StartImportJobRequest request)
        {
            try
            {
                ...
                Guid jobId = await _importRepository.CreateImportJobAsync(userId.Value, request);
        ...
        public async Task<ActionResult<List<RecipeVersionDto>>> GetVersions(Guid recipeId)
        {
            try
            {
                List<RecipeVersionDto> versions = await _importRepository.GetRecipeVersionsAsync(recipeId);
                return Ok(versions);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<RecipeForkDto>>> GetForks(Guid recipeId)
        {
            try
            {
                List<RecipeForkDto> forks = await _importRepository.GetRecipeForksAsync(recipeId);
                return Ok(forks);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<RecipeExportHistoryDto>>> GetExportHistory([FromQuery] int limit = 50)
        {
            try
            {
                ...
                List<RecipeExportHistoryDto> exports = await _importRepository.GetUserExportHistoryAsync(userId.Value, limit);
                return Ok(exports);
            }
            catch (Exception ex)
            {
