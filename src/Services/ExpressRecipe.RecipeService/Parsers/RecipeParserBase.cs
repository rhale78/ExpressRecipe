...
namespace ExpressRecipe.RecipeService.Parsers
{
    ...
    public abstract class RecipeParserBase : IRecipeParser
    {
        ...
        protected (decimal? quantity, string? unit, string remaining) ParseQuantityAndUnit(string text)
        {
            ...
            Match match = Regex.Match(text, quantityPattern);
            ...
            Match unitMatch = Regex.Match(remaining, unitPattern);
        ...
        protected decimal? ParseQuantity(string quantityStr)
        {
            ...
            Match mixedMatch = Regex.Match(quantityStr, @"(\d+)\s+(\d+)/(\d+)");
            ...
            Match fractionMatch = Regex.Match(quantityStr, @"(\d+)/(\d+)");
            ...
            // Handle decimal: "2.5"
            return decimal.TryParse(quantityStr, out var result) ? result : null;
        }
        ...
        protected (string ingredient, string? preparation) ExtractPreparation(string text)
        {
            ...
            Match parenMatch = Regex.Match(text, @"^(.+?)\s*\((.+?)\)\s*$");
        ...
        protected int? ParseTime(string? text)
        {
            ...
            // Match hours
            Match hourMatch = Regex.Match(text, @"(\d+(?:\.\d+)?)\s*(?:hour|hr|hrs?|h)\b", RegexOptions.IgnoreCase);
            ...
            // Match minutes
            Match minMatch = Regex.Match(text, @"(\d+)\s*(?:minute|min|mins?|m)\b", RegexOptions.IgnoreCase);
        ...
        protected (int? temperature, string? unit) ParseTemperature(string? text)
        {
            ...
            Match match = Regex.Match(text, @"(\d+)\s*(?:°|degrees?)?\s*([FCfc])", RegexOptions.IgnoreCase);
