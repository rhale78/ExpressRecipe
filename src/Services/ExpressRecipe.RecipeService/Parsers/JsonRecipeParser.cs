...
namespace ExpressRecipe.RecipeService.Parsers
{
    ...
    public class JsonRecipeParser : RecipeParserBase
    {
        ...
        public override async Task<List<ParsedRecipe>> ParseAsync(string content, ParserContext context)
        {
            ...
            try
            {
                ...
                if (root.ValueKind == JsonValueKind.Array)
                {
                    ...
                    {
                        ParsedRecipe? recipe = ParseRecipeElement(element);
                        if (recipe != null)
                        {
                            recipes.Add(recipe);
                        }
                ...
                {
                    ParsedRecipe? recipe = ParseRecipeElement(root);
                    if (recipe != null)
                    {
                        recipes.Add(recipe);
                    }
        ...
        private ParsedRecipe? ParseRecipeElement(JsonElement element)
        {
            ...
            // Instructions
            if (element.TryGetProperty("instructions", out JsonElement instructionsElement))
            {
                ParseInstructions(instructionsElement, recipe);
            }
            else if (element.TryGetProperty("steps", out JsonElement stepsElement))
            {
        ...
        private void ParseIngredients(JsonElement ingredientsElement, ParsedRecipe recipe, ref int order)
        {
            ...
            {
                foreach (JsonElement item in ingredientsElement.EnumerateArray())
                {
                    if (item.ValueKind == JsonValueKind.String)
                    {
                        // Simple string ingredient
                        ParsedIngredient? ingredient = ParseIngredientString(item.GetString() ?? "", order++);
                        if (ingredient != null)
                        {
        ...
        private void ParseInstructions(JsonElement instructionsElement, ParsedRecipe recipe)
        {
            ...
            {
                foreach (JsonElement item in instructionsElement.EnumerateArray())
                {
                    if (item.ValueKind == JsonValueKind.String)
                    {
