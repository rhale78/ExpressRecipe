...
namespace ExpressRecipe.RecipeService.CQRS.Queries
{
    ...
    public class GetRecipeDetailsQueryHandler : IQueryHandler<GetRecipeDetailsQuery, RecipeDetailsDto?>
    {
        ...
        public async Task<RecipeDetailsDto?> HandleAsync(GetRecipeDetailsQuery query, CancellationToken cancellationToken = default)
        {
            ...
            RecipeDetailsDto? cached = await _cacheService.GetAsync<RecipeDetailsDto>(cacheKey);
            if (cached != null)
            {
            ...
            // Get from database
            RecipeDetailsDto? recipe = await _repository.GetRecipeDetailsAsync(query.RecipeId);
            if (recipe == null)
            {
            ...
            // Get related data
            List<string> categories = await _repository.GetRecipeCategoriesAsync(query.RecipeId);
            List<string> tags = await _repository.GetRecipeTagsAsync(query.RecipeId);
            List<Shared.DTOs.Recipe.RecipeIngredientDto> ingredients = await _repository.GetIngredientsAsync(query.RecipeId);
            List<RecipeInstructionDto> instructions = await _repository.GetInstructionsAsync(query.RecipeId);
            RecipeNutritionDto? nutrition = await _repository.GetNutritionAsync(query.RecipeId);
            (decimal AverageRating, int RatingCount) rating = await _repository.GetAverageRatingAsync(query.RecipeId);
