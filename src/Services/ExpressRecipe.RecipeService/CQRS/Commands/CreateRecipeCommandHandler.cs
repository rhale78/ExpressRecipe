...
namespace ExpressRecipe.RecipeService.CQRS.Commands
{
    ...
    public class CreateRecipeCommandHandler : ICommandHandler<CreateRecipeCommand, Guid>
    {
        ...
        public async Task<Guid> HandleAsync(CreateRecipeCommand command, CancellationToken cancellationToken = default)
        {
            ...
            // Create recipe
            Guid recipeId = await _repository.CreateRecipeAsync(
                command.UserId,
                command.Name,
                command.Description,
            ...
            // Add ingredients
            foreach (IngredientItem ingredient in command.Ingredients)
            {
            ...
            // Add instructions
            foreach (InstructionStep instruction in command.Instructions)
            {
