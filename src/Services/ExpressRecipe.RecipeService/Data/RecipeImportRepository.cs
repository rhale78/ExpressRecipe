...
namespace ExpressRecipe.RecipeService.Data
{
    ...
    public class RecipeImportRepository : SqlHelper, IRecipeImportRepository
    {
        ...
        public async Task<RecipeImportSourceDto?> GetImportSourceByIdAsync(Guid id)
        {
            ...
            List<RecipeImportSourceDto> results = await ExecuteReaderAsync(
                sql,
                reader => new RecipeImportSourceDto
                {
                    Id = GetGuid(reader, "Id"),
        ...
        public async Task<RecipeImportJobDto?> GetImportJobByIdAsync(Guid id)
        {
            ...
            List<RecipeImportJobDto> results = await ExecuteReaderAsync(
                sql,
            ...
            RecipeImportJobDto? job = results.FirstOrDefault();
        ...
        public async Task<RecipeVersionDto?> GetRecipeVersionByIdAsync(Guid id)
        {
            ...
            List<RecipeVersionDto> results = await ExecuteReaderAsync(
                sql,
                reader => new RecipeVersionDto
                {
                    Id = GetGuid(reader, "Id"),
        ...
        public async Task<RecipeCollectionDto?> GetCollectionByIdAsync(Guid id, bool includeItems = true)
        {
            ...
            List<RecipeCollectionDto> results = await ExecuteReaderAsync(
                sql,
            ...
            RecipeCollectionDto? collection = results.FirstOrDefault();
        ...
        public async Task<List<RecipeCollectionDto>> GetUserCollectionsAsync(Guid userId, bool includeItemCounts = true)
        {
            ...
            List<RecipeCollectionDto> collections = await ExecuteReaderAsync(
                sql,
                reader => new RecipeCollectionDto
                {
                    Id = GetGuid(reader, "Id"),
            ...
            {
                foreach (RecipeCollectionDto? collection in collections)
                {
        ...
        {
            // Verify ownership
            RecipeCollectionDto? collection = await GetCollectionByIdAsync(collectionId, false);
        ...
        {
            Guid id = Guid.NewGuid();
            DateTime expiresAt = DateTime.UtcNow.AddDays(7); // Exports expire after 7 days
