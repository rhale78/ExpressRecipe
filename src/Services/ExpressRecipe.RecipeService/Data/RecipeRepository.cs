...
namespace ExpressRecipe.RecipeService.Data
{
    ...
    public class RecipeRepository : SqlHelper, IRecipeRepository
    {
        ...
        {
            CreateRecipeRequest request = new ExpressRecipe.Shared.DTOs.Recipe.CreateRecipeRequest
            {
                Name = name,
        ...
        public async Task<List<ExpressRecipe.Shared.DTOs.Recipe.RecipeDto>> SearchRecipesAsync(string searchTerm, int limit = 50, int offset = 0)
        {
            ...
            List<RecipeDto> results = await ExecuteReaderAsync(sql, reader => new ExpressRecipe.Shared.DTOs.Recipe.RecipeDto
            {
        ...
        public async Task<List<string>> GetRecipeCategoriesAsync(Guid recipeId)
        {
            ...
            List<string> categories = await ExecuteReaderAsync(sql, reader => reader.GetString(reader.GetOrdinal("Name")), new SqlParameter("@RecipeId", recipeId));
            return categories;
        }
        ...
        public async Task<List<string>> GetRecipeTagsAsync(Guid recipeId)
        {
            ...
            List<string> tags = await ExecuteReaderAsync(sql, reader => reader.GetString(reader.GetOrdinal("Name")), new SqlParameter("@RecipeId", recipeId));
            return tags;
        }
        ...
        public async Task<List<ExpressRecipe.Shared.DTOs.Recipe.RecipeIngredientDto>> GetIngredientsAsync(Guid recipeId)
        {
            ...
            List<Shared.DTOs.Recipe.RecipeIngredientDto> ingredients = await ExecuteReaderAsync(sql, reader => new ExpressRecipe.Shared.DTOs.Recipe.RecipeIngredientDto
            {
                Id = reader.GetGuid(reader.GetOrdinal("Id")),
        ...
        public async Task<List<CQ.RecipeInstructionDto>> GetInstructionsAsync(Guid recipeId)
        {
            ...
            List<RecipeInstructionDto> instructions = await ExecuteReaderAsync(sql, reader => new CQ.RecipeInstructionDto
            {
                Id = reader.GetGuid(reader.GetOrdinal("Id")),
        ...
        public async Task<CQ.RecipeNutritionDto?> GetNutritionAsync(Guid recipeId)
        {
            ...
            List<CQ.RecipeNutritionDto> results = await ExecuteReaderAsync(sql, reader => new CQ.RecipeNutritionDto
            {
        ...
        public async Task<CQ.RecipeDetailsDto?> GetRecipeDetailsAsync(Guid recipeId)
        {
            ...
            List<RecipeDetailsDto> results = await ExecuteReaderAsync(sql, reader => new CQ.RecipeDetailsDto
            {
                Id = reader.GetGuid(reader.GetOrdinal("Id")),
        ...
        public async Task<Guid> CreateRecipeAsync(ExpressRecipe.Shared.DTOs.Recipe.CreateRecipeRequest request, Guid createdBy)
        {
            ...
            if (totalTime == 0)
            {
                totalTime = request.TotalTimeMinutes ?? 0;
            }

            Guid recipeId = await ExecuteScalarAsync<Guid>(sql,
        ...
        public async Task AddRecipeTagAsync(Guid recipeId, string tagName)
        {
            ...
            await AddRecipeTagsAsync(recipeId, [tagName]);
        }
        ...
        public async Task AddRecipeIngredientsAsync(Guid recipeId, List<ExpressRecipe.Shared.DTOs.Recipe.RecipeIngredientDto> ingredients, Guid? createdBy = null)
        {
            ...
            try
            {
                foreach (Shared.DTOs.Recipe.RecipeIngredientDto ingredient in ingredients)
                {
        ...
        public async Task AddRecipeAllergensAsync(Guid recipeId, List<ExpressRecipe.Shared.DTOs.Recipe.RecipeAllergenWarningDto> allergens)
        {
            ...
            try
            {
                foreach (RecipeAllergenWarningDto allergen in allergens)
                {
                    using SqlCommand command = new SqlCommand(sql, connection, transaction);
                    command.Parameters.AddWithValue("@RecipeId", recipeId);
        ...
        public async Task AddRecipeTagsAsync(Guid recipeId, List<string> tagNames)
        {
            ...
            try
            {
                foreach (var tagName in tagNames.Distinct())
                {
                    ...
                    Guid tagId;
                    using (SqlCommand command = new SqlCommand(getTagSql, connection, transaction))
                    {
                        ...
                        if (result == null)
                        {
                            ...
                            using SqlCommand createCommand = new SqlCommand(createTagSql, connection, transaction);
                    ...
                    using (SqlCommand command = new SqlCommand(mappingSql, connection, transaction))
                    {
                        command.Parameters.AddWithValue("@RecipeId", recipeId);
        ...
        public async Task<ExpressRecipe.Shared.DTOs.Recipe.RecipeDto?> FindDuplicateRecipeAsync(string name, Guid authorId)
        {
            ...
            List<RecipeDto> recipes = await ExecuteReaderAsync(sql, reader => new ExpressRecipe.Shared.DTOs.Recipe.RecipeDto
            {
                Id = reader.GetGuid(reader.GetOrdinal("Id")),
                Name = reader.GetString(reader.GetOrdinal("Name")),
                AuthorId = reader.GetGuid(reader.GetOrdinal("AuthorId")),
