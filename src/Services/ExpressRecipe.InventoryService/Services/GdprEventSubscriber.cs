...
/// <summary>
/// Subscribes to <see cref="GdprDeleteEvent"/> and hard-deletes all inventory data
/// owned by the user (items, history, alerts, usage predictions, storage locations).
/// </summary>
public sealed class GdprEventSubscriber : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("[InventoryService] Subscribed to GDPR delete events");
    }
    ...
    {
        _logger.LogWarning(
            "[InventoryService] GDPR delete received for user {UserId} (requestId {RequestId})",
        ...
        try
        {
            ...
            _logger.LogInformation(
                "[InventoryService] GDPR delete complete for user {UserId}", evt.UserId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex,
                "[InventoryService] Error processing GDPR delete for user {UserId}", evt.UserId);
        }
    }
}
