...
namespace ExpressRecipe.InventoryService.Controllers
{
    ...
    public class InventoryController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetInventory()
        {
            ...
            List<InventoryItemDto> items = await _repository.GetUserInventoryAsync(userId);
            return Ok(items);
        }
        ...
        public async Task<IActionResult> AddItem([FromBody] AddInventoryItemRequest request)
        {
            ...
            Guid itemId = await _repository.AddInventoryItemAsync(
                userId,
                request.ProductId,
                request.CustomName,
            ...
            InventoryItemDto? item = await _repository.GetInventoryItemAsync(itemId, userId);
        ...
        public async Task<IActionResult> GetItem(Guid id)
        {
            ...
            InventoryItemDto? item = await _repository.GetInventoryItemAsync(id, userId);
        ...
        public async Task<IActionResult> GetExpiringItems([FromQuery] int daysAhead = 7)
        {
            ...
            List<InventoryItemDto> items = await _repository.GetExpiringItemsAsync(userId, daysAhead);
            return Ok(items);
        }
        ...
        public async Task<IActionResult> GetStorageLocations()
        {
            ...
            List<StorageLocationDto> locations = await _repository.GetStorageLocationsAsync(userId);
            return Ok(locations);
        }
        ...
        public async Task<IActionResult> CreateStorageLocation([FromBody] CreateStorageLocationRequest request)
        {
            ...
            Guid locationId = await _repository.CreateStorageLocationAsync(userId, request.Name, request.Description, request.Temperature);
            List<StorageLocationDto> locations = await _repository.GetStorageLocationsAsync(userId);
