...
namespace ExpressRecipe.InventoryService.Data
{
    public class InventoryRepository : IInventoryRepository
    {
            ...
            string? barcode, decimal? price, string? store)
        {
            ...
            Guid itemId = (Guid)await command.ExecuteScalarAsync()!;
        ...
        public async Task UpdateInventoryQuantityAsync(Guid itemId, decimal newQuantity, string actionType, string? reason)
        {
            ...
            try
            {
                ...
                decimal oldQuantity;
                Guid userId;
                await using (SqlCommand command = new SqlCommand(getQuantitySql, connection, transaction))
                {
                    command.Parameters.AddWithValue("@ItemId", itemId);
                    await using SqlDataReader reader = await command.ExecuteReaderAsync();
                ...
                // Update quantity
                await using (SqlCommand command = new SqlCommand(updateSql, connection, transaction))
                {
                    command.Parameters.AddWithValue("@ItemId", itemId);
        ...
        public async Task<Guid> CreateStorageLocationAsync(Guid userId, string name, string? description, string? temperature)
        {
            ...
            Guid locationId = (Guid)await command.ExecuteScalarAsync()!;
        ...
        public async Task<List<UsagePredictionDto>> GetUsagePredictionsAsync(Guid userId)
        {
            ...
            List<UsagePredictionDto> predictions = [];
            await using SqlDataReader reader = await command.ExecuteReaderAsync();
            while (await reader.ReadAsync())
            {
            ...
            SqlTransaction? transaction = null)
        {
            ...
            if (transaction != null)
            {
                await using SqlCommand command = new SqlCommand(sql, transaction.Connection, transaction);
