    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register([FromBody] RegisterRequest request)
        {
            try
            {
                ...
                Models.AuthUser? existingUser = await _repository.GetUserByEmailAsync(request.Email);
                ...
                Guid userId = await _repository.CreateUserAsync(
                    request.Email,
                    passwordHash,
                    request.FirstName ?? "",
                    request.LastName ?? ""
                ...
                // Get created user
                Models.AuthUser? user = await _repository.GetUserByIdAsync(userId);
                if (user == null)
                {
                ...
                DateTime refreshTokenExpiration = _tokenService.GetRefreshTokenExpiration();
                ...
                try
                {
                    CreateUserProfileForNewUserRequest profileRequest = new ExpressRecipe.Shared.DTOs.User.CreateUserProfileForNewUserRequest
                    {
                        UserId = userId,
                    ...
                    HttpClient userServiceClient = _httpClientFactory.CreateClient("UserService");
                    HttpResponseMessage profileResponse = await userServiceClient.PostAsJsonAsync("/api/userprofile/system/create", profileRequest);
        ...
        {
            // Get user by email
            Models.AuthUser? user = await _repository.GetUserByEmailAsync(request.Email);
            if (user == null)
            {
        ...
        public async Task<IActionResult> RefreshToken([FromBody] RefreshTokenRequest request)
        {
            ...
            (Guid userId, bool isValid) = await _repository.ValidateRefreshTokenAsync(request.RefreshToken);
