...
namespace ExpressRecipe.RecallService.Controllers
{
    ...
    public class RecallController : ControllerBase
    {
        ...
        {
            List<RecallDto> recalls = await _repository.GetRecentRecallsAsync(limit);
            return Ok(recalls);
        }
        ...
        {
            RecallDto? recall = await _repository.GetRecallAsync(id);
        ...
        {
            List<RecallDto> recalls = await _repository.SearchRecallsAsync(searchTerm ?? "", severity, startDate, endDate);
            return Ok(recalls);
        }
        ...
        {
            List<RecallProductDto> products = await _repository.GetRecallProductsAsync(id);
            return Ok(products);
        }
        ...
        public async Task<IActionResult> GetUserAlerts([FromQuery] bool unacknowledgedOnly = true)
        {
            ...
            List<RecallAlertDto> alerts = await _repository.GetUserAlertsAsync(userId, unacknowledgedOnly);
            return Ok(alerts);
        }
        ...
        {
            Guid userId = GetUserId();
            Guid subId = await _repository.SubscribeToRecallsAsync(userId, request.Category, request.Brand, request.Keyword);
        ...
        {
            Guid userId = GetUserId();
            List<RecallSubscriptionDto> subs = await _repository.GetUserSubscriptionsAsync(userId);
            return Ok(subs);
        }
