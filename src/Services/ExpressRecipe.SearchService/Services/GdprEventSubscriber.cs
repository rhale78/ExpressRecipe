...
namespace ExpressRecipe.SearchService.Services;

/// <summary>
/// Subscribes to <see cref="GdprDeleteEvent"/> and hard-deletes all search data
/// owned by the user (search history, preferences, recommendations).
/// </summary>
public sealed class GdprEventSubscriber : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("[SearchService] Subscribed to GDPR delete events");
    }
    ...
    {
        _logger.LogWarning(
            "[SearchService] GDPR delete received for user {UserId} (requestId {RequestId})",
            evt.UserId, evt.RequestId);

        try
        {
            using IServiceScope scope = _scopeFactory.CreateScope();
            ISearchRepository repo = scope.ServiceProvider.GetRequiredService<ISearchRepository>();
            await repo.DeleteUserDataAsync(evt.UserId, cancellationToken);
            _logger.LogInformation(
                "[SearchService] GDPR delete complete for user {UserId}", evt.UserId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex,
                "[SearchService] Error processing GDPR delete for user {UserId}", evt.UserId);
        }
    }
}
