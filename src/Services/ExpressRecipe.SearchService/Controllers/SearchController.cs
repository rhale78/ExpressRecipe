...
namespace ExpressRecipe.SearchService.Controllers
{
    ...
    public class SearchController : ControllerBase
    {
            ...
            [FromQuery] int offset = 0)
        {
            ...
            SearchResultDto results = await _repository.SearchAsync(query, entityType, category, null, limit, offset);
        ...
        {
            List<SearchSuggestionDto> suggestions = await _repository.GetSuggestionsAsync(q, limit);
        ...
        {
            Guid userId = GetUserId();
            List<SearchHistoryDto> history = await _repository.GetUserSearchHistoryAsync(userId, limit);
            return Ok(history);
        }
        ...
        {
            List<PopularSearchDto> popular = await _repository.GetPopularSearchesAsync(entityType, daysBack, limit);
            return Ok(popular);
        }
        ...
        {
            Guid userId = GetUserId();
            List<RecommendationDto> recommendations = await _repository.GetUserRecommendationsAsync(userId, entityType, limit);
        ...
        {
            Guid userId = GetUserId();
            Dictionary<string, string> prefs = await _repository.GetUserPreferencesAsync(userId);
            return Ok(prefs);
        }
    }
