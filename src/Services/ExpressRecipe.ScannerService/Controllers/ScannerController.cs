...
namespace ExpressRecipe.ScannerService.Controllers
{
    ...
    public class ScannerController : ControllerBase
    {
        ...
        public async Task<IActionResult> ScanBarcode([FromBody] ScanBarcodeRequest request)
        {
            ...
            Guid scanId = await _repository.CreateScanAsync(userId, request.Barcode, request.ProductId, request.WasRecognized, request.ScanType);

            // Check for allergens if product was recognized
            List<ScanAlertDto> alerts = [];
        ...
        {
            Guid userId = GetUserId();
            List<ScanHistoryDto> scans = await _repository.GetUserScansAsync(userId, limit);
            return Ok(scans);
        }
        ...
        {
            Guid userId = GetUserId();
            List<ScanAlertDto> alerts = await _repository.GetUserAlertsAsync(userId, unreadOnly);
            return Ok(alerts);
        }
        ...
        public async Task<IActionResult> ReportUnknownProduct([FromBody] ReportUnknownProductRequest request)
        {
            ...
            Guid unknownProductId = await _repository.ReportUnknownProductAsync(
        ...
        {
            List<UnknownProductDto> products = await _repository.GetUnknownProductsAsync(limit);
            return Ok(products);
        }
        ...
        {
            Guid userId = GetUserId();
            Guid ocrId = await _repository.SaveOCRResultAsync(
        ...
        {
            Guid userId = GetUserId();
            List<OCRResultDto> results = await _repository.GetUserOCRResultsAsync(userId, limit);
            return Ok(results);
        }
    }
