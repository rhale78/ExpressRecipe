    ...
    public class UPCDatabaseApiClient
    {
        ...
        public async Task<UPCDatabaseProduct?> GetProductByBarcodeAsync(string barcode)
        {
            try
            {
                ...
                HttpResponseMessage response = await _httpClient.GetAsync($"prod/trial/lookup?upc={barcode}");
                ...
                UPCDatabaseResponse? result = JsonSerializer.Deserialize<UPCDatabaseResponse>(json);
                ...
                UPCDatabaseProduct product = result.Items[0];
