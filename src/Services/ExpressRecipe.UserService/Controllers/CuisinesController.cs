...
namespace ExpressRecipe.UserService.Controllers
{
    ...
    public class CuisinesController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<CuisineDto>>> GetAll()
        {
            try
            {
                List<CuisineDto> cuisines = await _repository.GetAllAsync();
                return Ok(cuisines);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<CuisineDto>> GetById(Guid id)
        {
            try
            {
                CuisineDto? cuisine = await _repository.GetByIdAsync(id);

                return cuisine == null ? (ActionResult<CuisineDto>)NotFound(new { message = "Cuisine not found" }) : (ActionResult<CuisineDto>)Ok(cuisine);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<CuisineDto>>> Search([FromQuery] string q)
        {
            try
            {
                ...
                List<CuisineDto> cuisines = await _repository.SearchByNameAsync(q);
                return Ok(cuisines);
            }
            catch (Exception ex)
            {
