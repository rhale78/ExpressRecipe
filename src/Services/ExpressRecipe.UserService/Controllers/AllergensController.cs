...
namespace ExpressRecipe.UserService.Controllers
{
    ...
    public class AllergensController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<AllergenDto>>> GetAll()
        {
            try
            {
                List<AllergenDto> allergens = await _repository.GetAllAllergensAsync();
                return Ok(allergens);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<AllergenDto>> GetById(Guid id)
        {
            try
            {
                AllergenDto? allergen = await _repository.GetByIdAsync(id);

                return allergen == null ? (ActionResult<AllergenDto>)NotFound(new { message = "Allergen not found" }) : (ActionResult<AllergenDto>)Ok(allergen);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<AllergenDto>>> Search([FromQuery] string q)
        {
            try
            {
                ...
                List<AllergenDto> allergens = await _repository.SearchByNameAsync(q);
                return Ok(allergens);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<UserAllergenDto>>> GetMyAllergens()
        {
            try
            {
                ...
                List<UserAllergenDto> allergens = await _repository.GetUserAllergensAsync(userId);
                return Ok(allergens);
            }
        ...
        public async Task<ActionResult<UserAllergenDto>> AddMyAllergen([FromBody] AddUserAllergenRequest request)
        {
            try
            {
                ...
                AllergenDto? allergen = await _repository.GetByIdAsync(request.AllergenId);
                if (allergen == null)
                {
                    return NotFound(new { message = "Allergen not found" });
                }

                Guid userAllergenId = await _repository.AddUserAllergenAsync(userId, request, userId);
                List<UserAllergenDto> userAllergens = await _repository.GetUserAllergensAsync(userId);
                UserAllergenDto? addedAllergen = userAllergens.FirstOrDefault(ua => ua.Id == userAllergenId);
