    ...
    public class FamilyMembersController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<FamilyMemberDto>>> GetMyFamilyMembers()
        {
            try
            {
                ...
                List<FamilyMemberDto> familyMembers = await _repository.GetByPrimaryUserIdAsync(userId);
        ...
        public async Task<ActionResult<FamilyMemberDto>> GetById(Guid id)
        {
            ...
            {
                Guid userId = GetCurrentUserId();
                FamilyMemberDto? familyMember = await _repository.GetByIdAsync(id);
                ...
                return familyMember.PrimaryUserId != userId ? (ActionResult<FamilyMemberDto>)Forbid() : (ActionResult<FamilyMemberDto>)Ok(familyMember);
            }
            catch (UnauthorizedAccessException ex)
            {
                return Unauthorized(new { message = ex.Message });
        ...
        public async Task<ActionResult<FamilyMemberDto>> Create([FromBody] CreateFamilyMemberRequest request)
        {
            ...
            {
                Guid userId = GetCurrentUserId();
                Guid familyMemberId = await _repository.CreateAsync(userId, request, userId);
                FamilyMemberDto? familyMember = await _repository.GetByIdAsync(familyMemberId);
        ...
        public async Task<ActionResult<FamilyMemberDto>> Update(Guid id, [FromBody] UpdateFamilyMemberRequest request)
        {
            try
            {
                ...
                // Verify ownership
                FamilyMemberDto? existingMember = await _repository.GetByIdAsync(id);
