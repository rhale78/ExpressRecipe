...
namespace ExpressRecipe.UserService.Controllers
{
    ...
    public class ReportsController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<ReportTypeDto>>> GetReportTypes()
        {
            try
            {
                List<ReportTypeDto> reportTypes = await _reportsRepository.GetReportTypesAsync();
        ...
        public async Task<ActionResult<ReportTypeDto>> GetReportType(Guid id)
        {
            try
            {
                ReportTypeDto? reportType = await _reportsRepository.GetReportTypeByIdAsync(id);

                return reportType == null ? (ActionResult<ReportTypeDto>)NotFound(new { message = "Report type not found" }) : (ActionResult<ReportTypeDto>)Ok(reportType);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<SavedReportDto>>> GetSavedReports()
        {
            try
            {
                ...
                List<SavedReportDto> savedReports = await _reportsRepository.GetUserSavedReportsAsync(userId.Value);
        ...
        public async Task<ActionResult<SavedReportDto>> GetSavedReport(Guid id)
        {
            try
            {
                SavedReportDto? savedReport = await _reportsRepository.GetSavedReportByIdAsync(id);
                ...
                Guid? userId = GetCurrentUserId();
                return savedReport.UserId != userId ? (ActionResult<SavedReportDto>)Forbid() : (ActionResult<SavedReportDto>)Ok(savedReport);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<Guid>> CreateSavedReport([FromBody] CreateSavedReportRequest request)
        {
            try
            {
                ...
                Guid savedReportId = await _reportsRepository.CreateSavedReportAsync(userId.Value, request);
        ...
        public async Task<ActionResult> UpdateSavedReport(Guid id, [FromBody] UpdateSavedReportRequest request)
        {
            try
            {
                ...
                return !success ? NotFound(new { message = "Saved report not found" }) : NoContent();
            }
            catch (Exception ex)
            {
            ...
            [FromQuery] int pageSize = 50)
        {
            try
            {
                ...
                List<ReportHistoryDto> history = await _reportsRepository.GetUserReportHistoryAsync(userId.Value, pageNumber, pageSize);
                return Ok(history);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<Guid>> CreateReportHistory([FromBody] CreateReportHistoryRequest request)
        {
            try
            {
                ...
                Guid historyId = await _reportsRepository.CreateReportHistoryAsync(userId.Value, request);
