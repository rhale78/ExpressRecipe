...
namespace ExpressRecipe.UserService.Controllers
{
    ...
    public class SubscriptionsController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<SubscriptionTierDto>>> GetTiers()
        {
            try
            {
                List<SubscriptionTierDto> tiers = await _subscriptionRepository.GetSubscriptionTiersAsync();
                return Ok(tiers);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<SubscriptionTierDto>> GetTier(Guid id)
        {
            try
            {
                SubscriptionTierDto? tier = await _subscriptionRepository.GetSubscriptionTierByIdAsync(id);

                return tier == null ? (ActionResult<SubscriptionTierDto>)NotFound(new { message = "Subscription tier not found" }) : (ActionResult<SubscriptionTierDto>)Ok(tier);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<UserSubscriptionDto>> GetCurrentSubscription()
        {
            try
            {
                ...
                UserSubscriptionDto? subscription = await _subscriptionRepository.GetUserSubscriptionAsync(userId.Value);

                if (subscription == null)
                {
                    // User has no subscription, return Free tier info
                    SubscriptionTierDto? freeTier = await _subscriptionRepository.GetSubscriptionTierByNameAsync("Free");
                    return Ok(new
                    {
                        tier = freeTier,
                        status = "Free",
        ...
        public async Task<ActionResult<Guid>> Subscribe([FromBody] SubscribeRequest request)
        {
            try
            {
                ...
                SubscriptionTierDto? tier = await _subscriptionRepository.GetSubscriptionTierByIdAsync(request.SubscriptionTierId);
                if (tier == null)
                {
                ...
                Guid subscriptionId = await _subscriptionRepository.SubscribeAsync(userId.Value, request);
        ...
        public async Task<ActionResult<List<SubscriptionHistoryDto>>> GetHistory()
        {
            try
            {
                ...
                List<SubscriptionHistoryDto> history = await _subscriptionRepository.GetSubscriptionHistoryAsync(userId.Value);
                return Ok(history);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<Dictionary<string, bool>>> GetFeatureAccess()
        {
            try
            {
                ...
                Dictionary<string, bool> features = await _subscriptionRepository.GetFeatureAccessMapAsync(userId.Value);
                return Ok(features);
            }
            catch (Exception ex)
            {
