    ...
    public class PointsController : ControllerBase
    {
        ...
        public async Task<ActionResult<UserPointsSummaryDto>> GetSummary()
        {
            try
            {
                ...
                UserPointsSummaryDto summary = await _pointsRepository.GetUserPointsSummaryAsync(userId.Value);
                return Ok(summary);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<PointTransactionDto>>> GetTransactions([FromQuery] int limit = 50)
        {
            try
            {
                ...
                List<PointTransactionDto> transactions = await _pointsRepository.GetUserTransactionsAsync(userId.Value, limit);
            ...
            [FromQuery] int limit = 50)
        {
            try
            {
                ...
                List<UserContributionDto> contributions = await _pointsRepository.GetUserContributionsAsync(userId.Value, approvedOnly, limit);
        ...
        public async Task<ActionResult<List<ContributionTypeDto>>> GetContributionTypes([FromQuery] bool activeOnly = true)
        {
            try
            {
                List<ContributionTypeDto> types = await _pointsRepository.GetContributionTypesAsync(activeOnly);
                return Ok(types);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<RewardItemDto>>> GetRewards()
        {
            try
            {
                List<RewardItemDto> rewards = await _pointsRepository.GetActiveRewardsAsync();
                return Ok(rewards);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<RewardItemDto>> GetReward(Guid id)
        {
            try
            {
                RewardItemDto? reward = await _pointsRepository.GetRewardByIdAsync(id);

                return reward == null ? (ActionResult<RewardItemDto>)NotFound(new { message = "Reward not found" }) : (ActionResult<RewardItemDto>)Ok(reward);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<RewardItemDto>>> GetRedeemedRewards([FromQuery] bool activeOnly = true)
        {
            try
            {
                ...
                List<RewardItemDto> rewards = await _pointsRepository.GetUserRedeemedRewardsAsync(userId.Value, activeOnly);
                return Ok(rewards);
            }
            catch (Exception ex)
            {
