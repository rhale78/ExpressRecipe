    ...
    public class FriendsController : ControllerBase
    {
        ...
        public async Task<ActionResult<FriendsSummaryDto>> GetSummary()
        {
            try
            {
                ...
                FriendsSummaryDto summary = await _friendsRepository.GetFriendsSummaryAsync(userId.Value);
                return Ok(summary);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<UserFriendDto>>> GetFriends([FromQuery] string? status = null)
        {
            try
            {
                ...
                List<UserFriendDto> friends = await _friendsRepository.GetUserFriendsAsync(userId.Value, status);
                return Ok(friends);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<Guid>> SendFriendRequest([FromBody] SendFriendRequestRequest request)
        {
            try
            {
                ...
                Guid friendRequestId = await _friendsRepository.SendFriendRequestAsync(
                    userId.Value,
        ...
        public async Task<ActionResult<Guid>> SendInvitation([FromBody] InviteFriendRequest request)
        {
            try
            {
                ...
                Guid invitationId = await _friendsRepository.SendInvitationAsync(
                    userId.Value,
        ...
        public async Task<ActionResult<List<FriendInvitationDto>>> GetInvitations()
        {
            try
            {
                ...
                List<FriendInvitationDto> invitations = await _friendsRepository.GetUserInvitationsAsync(userId.Value);
        ...
        public async Task<ActionResult<FriendInvitationDto>> GetInvitationByCode(string invitationCode)
        {
            try
            {
                FriendInvitationDto? invitation = await _friendsRepository.GetInvitationByCodeAsync(invitationCode);

                return invitation == null ? (ActionResult<FriendInvitationDto>)NotFound(new { message = "Invitation not found" }) : (ActionResult<FriendInvitationDto>)Ok(invitation);
            }
            catch (Exception ex)
            {
