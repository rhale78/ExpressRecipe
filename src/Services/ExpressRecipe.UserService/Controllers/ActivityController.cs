    ...
    public class ActivityController : ControllerBase
    {
            ...
            [FromQuery] int pageSize = 50)
        {
            try
            {
                ...
                List<UserActivityDto> activities = await _activityRepository.GetUserActivityAsync(userId.Value, pageNumber, pageSize);
        ...
        public async Task<ActionResult<List<UserActivityDto>>> GetRecentActivity([FromQuery] int days = 7)
        {
            try
            {
                ...
                List<UserActivityDto> activities = await _activityRepository.GetRecentActivityAsync(userId.Value, days);
                return Ok(activities);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<UserActivityDto>>> GetActivityByType(string activityType)
        {
            try
            {
                ...
                List<UserActivityDto> activities = await _activityRepository.GetActivityByTypeAsync(userId.Value, activityType);
                return Ok(activities);
            }
            catch (Exception ex)
            {
            ...
            [FromQuery] DateTime? endDate = null)
        {
            try
            {
                ...
                UserActivitySummaryDto summary = await _activityRepository.GetActivitySummaryAsync(userId.Value, startDate, endDate);
                return Ok(summary);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<Dictionary<string, int>>> GetActivityCounts([FromQuery] int days = 30)
        {
            try
            {
                ...
                Dictionary<string, int> counts = await _activityRepository.GetActivityCountsByTypeAsync(userId.Value, days);
                return Ok(counts);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<Guid>> LogActivity([FromBody] LogActivityRequest request)
        {
            try
            {
                ...
                Guid activityId = await _activityRepository.LogActivityAsync(userId.Value, request);
