...
namespace ExpressRecipe.UserService.Controllers
{
    ...
    public class DietaryRestrictionsController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<DietaryRestrictionDto>>> GetAll()
        {
            try
            {
                List<DietaryRestrictionDto> restrictions = await _repository.GetAllRestrictionsAsync();
        ...
        public async Task<ActionResult<DietaryRestrictionDto>> GetById(Guid id)
        {
            try
            {
                DietaryRestrictionDto? restriction = await _repository.GetByIdAsync(id);

                return restriction == null ? (ActionResult<DietaryRestrictionDto>)NotFound(new { message = "Dietary restriction not found" }) : (ActionResult<DietaryRestrictionDto>)Ok(restriction);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<DietaryRestrictionDto>>> GetByType(string type)
        {
            try
            {
                List<DietaryRestrictionDto> restrictions = await _repository.GetByTypeAsync(type);
                return Ok(restrictions);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<List<UserDietaryRestrictionDto>>> GetMyRestrictions()
        {
            try
            {
                ...
                List<UserDietaryRestrictionDto> restrictions = await _repository.GetUserRestrictionsAsync(userId);
                return Ok(restrictions);
            }
        ...
        public async Task<ActionResult<UserDietaryRestrictionDto>> AddMyRestriction([FromBody] AddUserDietaryRestrictionRequest request)
        {
            try
            {
                ...
                DietaryRestrictionDto? restriction = await _repository.GetByIdAsync(request.DietaryRestrictionId);
                if (restriction == null)
                {
                    return NotFound(new { message = "Dietary restriction not found" });
                }

                Guid userRestrictionId = await _repository.AddUserRestrictionAsync(userId, request, userId);
                List<UserDietaryRestrictionDto> userRestrictions = await _repository.GetUserRestrictionsAsync(userId);
                UserDietaryRestrictionDto? addedRestriction = userRestrictions.FirstOrDefault(ur => ur.Id == userRestrictionId);
