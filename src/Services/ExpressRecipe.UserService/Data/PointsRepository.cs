...
namespace ExpressRecipe.UserService.Data
{
    ...
    public class PointsRepository : SqlHelper, IPointsRepository
    {
        ...
        public async Task<ContributionTypeDto?> GetContributionTypeByIdAsync(Guid id)
        {
            ...
            List<ContributionTypeDto> results = await ExecuteReaderAsync(
                sql,
                reader => new ContributionTypeDto
                {
                    Id = GetGuid(reader, "Id"),
        ...
        public async Task<Guid> CreateContributionAsync(Guid userId, Guid contributionTypeId, Guid? referenceId, string? referenceType, string? notes = null)
        {
            ...
            ContributionTypeDto? contributionType = await GetContributionTypeByIdAsync(contributionTypeId);
        ...
        public async Task<bool> ApproveContributionAsync(Guid contributionId, Guid approvedBy, bool approve, string? rejectionReason = null)
        {
            ...
            UserContributionDto? contribution = await GetContributionByIdAsync(contributionId);
            ...
            ContributionTypeDto? contributionType = await GetContributionTypeByIdAsync(contribution.ContributionTypeId);
            if (contributionType == null)
            {
                return false;
            }
        ...
        public async Task<UserContributionDto?> GetContributionByIdAsync(Guid id)
        {
            ...
            List<UserContributionDto> results = await ExecuteReaderAsync(
                sql,
                reader => new UserContributionDto
                {
                    Id = GetGuid(reader, "Id"),
        ...
        public async Task<UserPointsSummaryDto> GetUserPointsSummaryAsync(Guid userId)
        {
            ...
            List<PointTransactionDto> recentTransactions = await GetUserTransactionsAsync(userId, 10);

            // Get recent contributions
            List<UserContributionDto> recentContributions = await GetUserContributionsAsync(userId, null, 10);
        ...
        public async Task<RewardItemDto?> GetRewardByIdAsync(Guid id)
        {
            ...
            List<RewardItemDto> results = await ExecuteReaderAsync(
                sql,
                reader => new RewardItemDto
                {
                    Id = GetGuid(reader, "Id"),
        ...
        {
            // Get reward
            RewardItemDto? reward = await GetRewardByIdAsync(rewardItemId);
            ...
            Guid redemptionId = Guid.NewGuid();
