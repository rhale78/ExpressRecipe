...
namespace ExpressRecipe.MenuItemService.Services;

/// <summary>
/// Subscribes to <see cref="GdprDeleteEvent"/> and hard-deletes all menu-item
...
public sealed class GdprEventSubscriber : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("[MenuItemService] Subscribed to GDPR delete events");
    }
    ...
    {
        _logger.LogWarning(
            "[MenuItemService] GDPR delete received for user {UserId} (requestId {RequestId})",
            evt.UserId, evt.RequestId);

        try
        {
            using IServiceScope scope = _scopeFactory.CreateScope();
            IMenuItemRepository repo = scope.ServiceProvider.GetRequiredService<IMenuItemRepository>();
            await repo.DeleteUserDataAsync(evt.UserId, cancellationToken);
            _logger.LogInformation(
                "[MenuItemService] GDPR delete complete for user {UserId}", evt.UserId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex,
                "[MenuItemService] Error processing GDPR delete for user {UserId}", evt.UserId);
        }
    }
}
