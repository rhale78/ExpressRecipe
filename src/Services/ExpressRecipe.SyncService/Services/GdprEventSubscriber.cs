...
namespace ExpressRecipe.SyncService.Services;

/// <summary>
/// Subscribes to <see cref="GdprDeleteEvent"/> and hard-deletes all sync data
/// owned by the user (sync queue, device registrations, conflicts).
/// </summary>
public sealed class GdprEventSubscriber : IHostedService
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("[SyncService] Subscribed to GDPR delete events");
    }
    ...
    {
        _logger.LogWarning(
            "[SyncService] GDPR delete received for user {UserId} (requestId {RequestId})",
            evt.UserId, evt.RequestId);

        try
        {
            using IServiceScope scope = _scopeFactory.CreateScope();
            ISyncRepository repo = scope.ServiceProvider.GetRequiredService<ISyncRepository>();
            await repo.DeleteUserDataAsync(evt.UserId, cancellationToken);
            _logger.LogInformation(
                "[SyncService] GDPR delete complete for user {UserId}", evt.UserId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex,
                "[SyncService] Error processing GDPR delete for user {UserId}", evt.UserId);
        }
    }
}
