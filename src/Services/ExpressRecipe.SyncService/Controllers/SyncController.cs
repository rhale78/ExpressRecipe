...
namespace ExpressRecipe.SyncService.Controllers
{
    ...
    public class SyncController : ControllerBase
    {
        ...
        public async Task<IActionResult> RegisterDevice([FromBody] RegisterDeviceRequest request)
        {
            ...
            Guid deviceId = await _repository.RegisterDeviceAsync(
        ...
        {
            Guid userId = GetUserId();
            List<DeviceRegistrationDto> devices = await _repository.GetUserDevicesAsync(userId);
            return Ok(devices);
        }
        ...
        public async Task<IActionResult> PushChanges([FromBody] PushChangesRequest request)
        {
            ...
            {
                Guid syncId = await _repository.CreateSyncMetadataAsync(
        ...
        {
            Guid userId = GetUserId();
            List<SyncMetadataDto> changes = await _repository.GetPendingSyncsAsync(userId, deviceId, since);
            return Ok(changes);
        }
        ...
        {
            Guid userId = GetUserId();
            List<SyncConflictDto> conflicts = await _repository.GetUnresolvedConflictsAsync(userId);
            return Ok(conflicts);
        }
        ...
        {
            Guid userId = GetUserId();
            SyncStatsDto stats = await _repository.GetSyncStatsAsync(userId, deviceId);
            return Ok(stats);
        }
    }
