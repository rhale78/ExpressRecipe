...
public sealed class RemlParser : IRecipeParser
{
    public string FormatName => "Reml";

    public bool CanParse(string text, string? fileExtension = null)
    {
        return text.Contains("<REML") || text.Contains("<reml");
    }
        ...
        try
        {
            ...
            foreach (XmlNode node in recipeNodes)
            {
                try
                {
                    ...
                    recipe.PrepTime = XmlParserHelper.GetElementText(node, "preptime") ?? XmlParserHelper.GetElementText(node, "prep-time");
                    recipe.CookTime = XmlParserHelper.GetElementText(node, "cooktime") ?? XmlParserHelper.GetElementText(node, "cook-time");
                ...
                catch (Exception ex) { LoggingHelper.LogRecipeError(null, errors, i, null, "Failed to parse REML recipe", ex); }
                i++;
            }
            result.Success = true;
        }
        catch (Exception ex) { LoggingHelper.LogBatchError(null, errors, "Failed to parse REML XML", ex); }
