...
public sealed class YumParser : IRecipeParser
{
    public string FormatName => "Yum";

    public bool CanParse(string text, string? fileExtension = null)
    {
        return text.Contains("<YumRecipes") || text.Contains("<yum");
    }
    ...
    public ParseResult Parse(string text, RecipeParseOptions? options = null)
    {
        ...
        try
        {
            ...
            foreach (XmlNode node in recipeNodes)
            {
                ...
                catch (Exception ex) { LoggingHelper.LogRecipeError(null, errors, i, null, "Failed to parse Yum recipe", ex); }
                i++;
            }
            result.Success = true;
        }
        catch (Exception ex) { LoggingHelper.LogBatchError(null, errors, "Failed to parse Yum XML", ex); }
