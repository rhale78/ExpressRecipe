...
namespace ExpressRecipe.Shared.Resilience
{
    ...
    public static class ResiliencePolicies
    {
        ...
        {
            AsyncRetryPolicy<HttpResponseMessage> retryPolicy = GetRetryPolicy(logger, policyName);
            AsyncCircuitBreakerPolicy<HttpResponseMessage> circuitBreakerPolicy = GetCircuitBreakerPolicy(logger, policyName);
            AsyncTimeoutPolicy<HttpResponseMessage> timeoutPolicy = GetTimeoutPolicy(logger, policyName);
        ...
        public static IAsyncPolicy<HttpResponseMessage> GetFullResiliencePolicy(ILogger logger, string policyName = "full")
        {
            ...
            AsyncCircuitBreakerPolicy<HttpResponseMessage> circuitBreakerPolicy = GetCircuitBreakerPolicy(logger, policyName);
            AsyncTimeoutPolicy<HttpResponseMessage> timeoutPolicy = GetTimeoutPolicy(logger, policyName);
            AsyncPolicy<HttpResponseMessage> bulkheadPolicy = GetBulkheadPolicy(logger, policyName);
