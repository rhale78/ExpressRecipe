...
public class HybridCacheServiceTests
{
    ...
    public async Task GetOrSetAsync_NullFactory_StillReturnsNull()
    {
        ...
        var result = await _cache.GetOrSetAsync<string?>(key, async (ct) => null);
    ...
    {
        var id = Guid.NewGuid();
        var key = CacheKeys.FormatKey(CacheKeys.CookbookById, id);
    ...
    public async Task CookbookBySlug_KeyPattern_SetAndGet()
    {
        ...
        var key = CacheKeys.FormatKey(CacheKeys.CookbookBySlug, slug);
    ...
    public async Task DifferentCookbookIds_HaveIndependentCacheEntries()
    {
        ...
        var key1 = CacheKeys.FormatKey(CacheKeys.CookbookById, id1);
        var key2 = CacheKeys.FormatKey(CacheKeys.CookbookById, id2);
    ...
    {
        var key = CacheKeys.FormatKey(CacheKeys.CookbookBySlug, "my-slug");
    ...
    {
        var id = Guid.NewGuid();
        var direct = CacheKeys.FormatKey(CacheKeys.CookbookById, id);
