...
namespace ExpressRecipe.Client.Shared.Services
{
    ...
    public class AdminApiClient : IAdminApiClient
    {
        ...
        {
            HttpClient client = await CreateClientAsync("ProductService");
            HttpResponseMessage response = await client.PostAsJsonAsync("/api/admin/import/usda", new { });
            return await response.Content.ReadFromJsonAsync<ImportStatusDto>();
        }

        public async Task<ImportStatusDto?> ImportFDARecallsAsync()
        {
            HttpClient client = await CreateClientAsync("RecallService");
            HttpResponseMessage response = await client.PostAsJsonAsync("/api/admin/import/fda", new { });
            return await response.Content.ReadFromJsonAsync<ImportStatusDto>();
        }
        ...
        {
            HttpClient client = await CreateClientAsync("ProductService");
            HttpResponseMessage response = await client.PostAsJsonAsync("/api/admin/import/openfoodfacts", new { });
            return await response.Content.ReadFromJsonAsync<ImportStatusDto>();
        }
        ...
        public async Task<ImportStatusDto?> GetImportStatusAsync(Guid importId)
        {
            ...
            {
                HttpClient client = await CreateClientAsync("ProductService");
                HttpResponseMessage response = await client.GetAsync($"/api/admin/import/status/{importId}");
        ...
        public async Task<List<ImportHistoryDto>> GetImportHistoryAsync()
        {
            ...
            {
                HttpClient client = await CreateClientAsync("ProductService");
                HttpResponseMessage response = await client.GetAsync("/api/admin/import/history");
                if (response.IsSuccessStatusCode)
                {
                    List<ImportHistoryDto>? productHistory = await response.Content.ReadFromJsonAsync<List<ImportHistoryDto>>();
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    List<ImportHistoryDto>? recallHistory = await response.Content.ReadFromJsonAsync<List<ImportHistoryDto>>();
