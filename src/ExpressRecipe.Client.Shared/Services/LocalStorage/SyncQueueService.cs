...
namespace ExpressRecipe.Client.Shared.Services.LocalStorage
{
    ...
    public class SyncQueueService
    {
        ...
        {
            List<SyncQueueItem> queue = await GetQueueAsync();

            // Check if item already exists
            SyncQueueItem? existing = queue.FirstOrDefault(x =>
        ...
        {
            List<SyncQueueItem> queue = await GetQueueAsync();
            SyncQueueItem? item = queue.FirstOrDefault(x => x.Id == itemId);
        ...
        {
            List<SyncQueueItem> queue = await GetQueueAsync();
            List<SyncQueueItem> pending = queue.Where(x => !x.IsSynced).ToList();
        ...
        {
            List<SyncQueueItem> pending = await GetPendingAsync();
            ...
            foreach (SyncQueueItem? item in pending.Take(100)) // Process in batches of 100
            {
                try
                {
                    // Call sync service for this item
                    Services.SyncResult result = await _syncClient.SyncEntityAsync(item.EntityType, item.EntityId);
        ...
        private async Task<List<SyncQueueItem>> GetQueueAsync()
        {
            try
            {
                List<SyncQueueItem>? queue = await _localStorage.GetItemAsync<List<SyncQueueItem>>(QueueKey);
                return queue ?? [];
            }
            catch
            {
