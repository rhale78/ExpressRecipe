...
public sealed class BatchUpdateTracker
{
    ...
    {
        var elapsed = CachedDateTimeUtc.UtcNow - _startTime;
        var summary = new BatchUpdateSummary
        {
    ...
    public override string ToString()
    {
        if (_totalAttempted == 0)
            return $"{_batchName}: No items processed";

        return $"{_batchName}: {_successfulUpdates}/{_totalAttempted} updated, {_skippedUpdates} skipped ({SkipPercentage:F1}%), {_failedUpdates} failed";
    }
}
