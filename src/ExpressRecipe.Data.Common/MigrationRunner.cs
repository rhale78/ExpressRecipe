...
namespace ExpressRecipe.Data.Common
{
    ...
    public class MigrationRunner
    {
            ...
            try
            {
                foreach (var batch in batches)
                {
                    ...
                    if (requiresNonTx)
                    {
                        ...
                        await using SqlCommand nonTxCommand = new SqlCommand(batch, connection);
                    ...
                    else
                    {
                        ...
                        await using SqlCommand command = new SqlCommand(batch, connection, tx);
        ...
        public async Task ApplyMigrationsAsync(IDictionary<string, string> migrations)
        {
            ...
            IOrderedEnumerable<KeyValuePair<string, string>> orderedMigrations = migrations.OrderBy(m => m.Key);

            foreach (KeyValuePair<string, string> migration in orderedMigrations)
            {
