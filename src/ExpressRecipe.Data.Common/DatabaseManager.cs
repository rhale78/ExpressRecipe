...
public class DatabaseManagementSettings
{
    ...
    public Dictionary<string, ServiceDatabaseSettings> Services { get; set; } = new();
}
...
public class DatabaseManager
{
    ...
    {
        if (_settings.Services.TryGetValue(serviceKey, out var serviceSettings))
        {
    ...
    public async Task DropDatabaseAsync(string databaseName)
    {
        try
        {
            ...
            await using var resetConnection = new SqlConnection(_serverConnectionString);
            await resetConnection.OpenAsync();

            await using (var resetCommand = new SqlCommand(resetSql, resetConnection)
            {
            ...
            await using var dropCommand = new SqlCommand(dropSql, resetConnection)
            {
                CommandTimeout = _settings.DatabaseOperationTimeout
            };
        ...
        catch (Exception ex)
        {
            ...
            try
            {
                ...
                await using var recoverConnection = new SqlConnection(_serverConnectionString);
                await recoverConnection.OpenAsync();
                await using var recoverCommand = new SqlCommand(recoverSql, recoverConnection)
                {
                    CommandTimeout = _settings.DatabaseOperationTimeout
                };
    ...
    {
        var settings = new DatabaseManagementSettings();
    ...
    private async Task DropAllTablesAsync(string databaseName)
    {
        ...
        await using var resetCmd = new SqlCommand(resetHistory, connection)
        {
            CommandTimeout = _settings.DatabaseOperationTimeout
        };
...
public static class DatabaseManagementExtensions
{
    ...
    {
        var settings = DatabaseManager.GetSettings(app.Configuration);
        ...
        var manager = new DatabaseManager(serverConnectionString, settings, logger);
