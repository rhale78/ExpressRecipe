    ...
    public abstract class SqlHelper
    {
            ...
            params DbParameter[] parameters)
        {
            return mapper == null
                ...
                : await ExecuteReaderAsync(sql, record => mapper((SqlDataReader)record), timeoutSeconds: 30, parameters);
        }
            ...
            params DbParameter[] parameters)
        {
            return mapper == null
                ? throw new ArgumentNullException(nameof(mapper))
                : await ExecuteReaderAsync(sql, record => mapper((SqlDataReader)record), timeoutSeconds, parameters);
        }
        ...
        protected async Task<T?> ExecuteReaderSingleAsync<T>(
            ...
            params DbParameter[] parameters)
        {
            List<T> results = await ExecuteReaderAsync(sql, mapper, parameters);
            ...
            params DbParameter[] parameters)
        {
            return mapper == null
                ? throw new ArgumentNullException(nameof(mapper))
                : await ExecuteReaderSingleAsync(sql, record => mapper((SqlDataReader)record), parameters);
        }
        ...
        {
            SqlParameter cloned = new SqlParameter
            {
            ...
            where TKey : notnull
        {
                ...
                : await ExecuteWithDeadlockRetryAsync(async () =>
            {
                ...
                await using (SqlCommand createCmd = new SqlCommand(createTempTableSql, connection))
                {
                ...
                await using (SqlCommand queryCmd = new SqlCommand(querySql, connection))
                {
                    ...
                    await using SqlDataReader reader = await queryCmd.ExecuteReaderAsync(cancellationToken);

                    while (await reader.ReadAsync(cancellationToken))
                    {
                        KeyValuePair<TKey, TValue> kvp = mapper(reader);
            ...
            params DbParameter[] parameters)
        {
            ...
            EntityChangeTracker tracker = new EntityChangeTracker(originalEntity);
            ...
            Type currentType = currentEntity.GetType();

            foreach (PropertyInfo prop in currentType.GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance))
            {
                if (prop.CanRead)
                {
            ...
            Type originalType = originalEntity.GetType();

            foreach (PropertyInfo prop in originalType.GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance))
            {
                if (!prop.CanRead)
                {
                    continue;
            ...
            params DbParameter[] parameters)
        {
            ...
            List<T>[] results = await Task.WhenAll(tasks);

            // Combine results from all partitions
            List<T> combined = new List<T>(results.Sum(r => r.Count));
