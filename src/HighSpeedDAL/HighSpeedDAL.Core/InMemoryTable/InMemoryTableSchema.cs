...
public sealed class InMemoryTableSchema
{
    ...
    public InMemoryTableSchema(string tableName)
    {
        ...
        _indexes = new List<InMemoryIndex>();
    ...
    private static string GetSqlType(Type type, int? maxLength)
    {
        ...
        if (type == typeof(byte[])) return maxLength.HasValue ? $"VARBINARY({maxLength})" : "VARBINARY(MAX)";
        if (type == typeof(string)) return maxLength.HasValue ? $"NVARCHAR({maxLength})" : "NVARCHAR(MAX)";
