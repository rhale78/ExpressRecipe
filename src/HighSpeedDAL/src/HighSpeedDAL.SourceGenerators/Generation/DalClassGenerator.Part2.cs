...
namespace HighSpeedDAL.SourceGenerators.Generation
{
    ...
    internal sealed partial class DalClassGenerator
    {
        private void GenerateInsertMethod(StringBuilder code)
        {
            ...
            // For Guid primary keys, generate the Guid before insert
            ...
            {
                code.AppendLine("        // Generate Guid ID if not already set");
            ...
            if (_metadata.IsAuditable)
            {
                ...
                {
                    // For non-auto-increment keys (Guid), just execute the insert without retrieving ID
                    code.AppendLine("        await ExecuteNonQueryAsync(");
                    code.AppendLine("            SQL_INSERT,");
                    code.AppendLine("            parameters,");
                    code.AppendLine("            transaction: null,");
                    code.AppendLine("            cancellationToken);");
        ...
        private void GenerateDuplicateKeyMatcher(StringBuilder code)
        {
            ...
            List<PropertyMetadata> uniqueIndexedProps = _metadata.Properties
                .Where(p => p.IsUniqueIndex || p.IsPrimaryKey)
                .ToList();

            // Also add composite indexes from Indexes collection
            List<IndexMetadata> compositeIndexes = _metadata.Indexes.Where(i => i.IsUnique).ToList();

            // Track which index names we've already added to avoid duplicates
            HashSet<string> addedIndexNames = [];
            ...
            else
            {
                ...
                foreach (PropertyMetadata prop in uniqueIndexedProps)
                {
                ...
                foreach (IndexMetadata index in compositeIndexes)
                {
                    // Skip if we've already added this index name
                    ...
                    for (int i = 0; i < index.ColumnNames.Count; i++)
                    {
                        ...
                        PropertyMetadata prop = _metadata.Properties.FirstOrDefault(p => p.ColumnName == colName || p.PropertyName == colName);
                        if (prop != null)
                        {
                ...
                foreach (PropertyMetadata? prop in uniqueIndexedProps.Take(3)) // Check first few unique props
                {
                    if (prop.IsNullable || prop.PropertyType == "string" || prop.PropertyType == "String")
                    {
                        code.AppendLine($"                    matches.AddRange(entities.Where(e => e.{prop.PropertyName} != null && e.{prop.PropertyName}.ToString() == duplicateValue));");
        ...
        private void GenerateMapFromReaderMethod(StringBuilder code)
        {
            ...
            HashSet<string> auditAndSoftDeleteProps =
            [
