...
namespace HighSpeedDAL.SourceGenerators.Parsing
{
    ...
    internal sealed class EntityParser
    {
        ...
        private static readonly Dictionary<string, (string contentHash, EntityMetadata metadata)> _schemaCache = [];
        ...
        public EntityMetadata? ParseEntity(ClassDeclarationSyntax classDeclaration)
        {
            ...
            foreach (IPropertySymbol property in properties)
            {
                ...
                if (propertyMetadata != null && !propertyMetadata.IsNotMapped)
                {
                    ...
                    if (propertyMetadata.IsPrimaryKey)
                    {
                        ...
                        // Update PrimaryKeyType based on property type
                        if (propertyMetadata.PropertyType == "Guid" || propertyMetadata.PropertyType == "System.Guid")
                        {
                            metadata.PrimaryKeyType = "Guid";
                        }
                        else if (propertyMetadata.PropertyType == "int" || propertyMetadata.PropertyType == "long")
                        {
                            metadata.PrimaryKeyType = "Int";
                        }
                    }
                    ...
                    if (propertyMetadata.IsIndexed)
                    {
                        ...
                        {
                            IndexName = indexName,
                            ColumnNames = [propertyMetadata.ColumnName],
            ...
            if (metadata.PrimaryKeyProperty == null)
            {
                ...
                if (existingId != null)
                {
                    ...
                    // Default convention: integer Id is auto-increment, Guid is not
                    if (existingId.PropertyType == "int" || existingId.PropertyType == "long")
                    {
                        existingId.IsAutoIncrement = true;
                        metadata.PrimaryKeyType = "Int"; // Update metadata to reflect int primary key
                    }
                    else if (existingId.PropertyType == "Guid" || existingId.PropertyType == "System.Guid")
                    {
                        existingId.IsAutoIncrement = false; // Guids are generated by app code
                        metadata.PrimaryKeyType = "Guid"; // Update metadata to reflect Guid primary key
                    }
                ...
                else
                {
                    ...
                    string idType = metadata.PrimaryKeyType == "Guid" ? "Guid" : "int";
                    bool isAutoIncrement = metadata.PrimaryKeyType != "Guid"; // Int is auto-increment, Guid is not
            ...
            private static bool TryGetCachedSchema(string cacheKey, string currentContentHash, out EntityMetadata? metadata)
            {
                lock (_cacheLock)
                {
                    if (_schemaCache.TryGetValue(cacheKey, out (string contentHash, EntityMetadata metadata) cachedEntry))
                    {
            ...
            private static EntityMetadata CloneMetadata(EntityMetadata source)
            {
                EntityMetadata clone = new EntityMetadata
                {
                    ...
                    PrimaryKeyType = source.PrimaryKeyType, // Copy PrimaryKeyType
                ...
                foreach (IndexMetadata idx in source.Indexes)
                {
                    clone.Indexes.Add(new IndexMetadata
                    {
                        ...
                        ColumnNames = idx.ColumnNames?.ToList() ?? [],
                ...
                foreach (NamedQueryMetadata nq in source.NamedQueries)
                {
                    ...
                    {
                        Name = nq.Name,
                        PropertyNames = nq.PropertyNames?.ToList() ?? [],
        ...
        private void ParseTableAttribute(AttributeData attribute, EntityMetadata metadata)
        {
            ...
            // Check for Schema and PrimaryKeyType named arguments
            foreach (KeyValuePair<string, TypedConstant> namedArg in attribute.NamedArguments)
            {
                ...
                else if (namedArg.Key == "PrimaryKeyType" && namedArg.Value.Value is int primaryKeyTypeValue)
                {
                    // Parse PrimaryKeyType enum: 0 = Int, 1 = Guid
                    metadata.PrimaryKeyType = primaryKeyTypeValue == 1 ? "Guid" : "Int";
                }
            }
        }
        ...
        {
            NamedQueryMetadata namedQuery = new NamedQueryMetadata();
            ...
            if (attribute.ConstructorArguments.Length > 1)
            {
                ...
                TypedConstant propertiesArg = attribute.ConstructorArguments[1];
                if (propertiesArg.Kind == TypedConstantKind.Array)
                {
                    foreach (TypedConstant propValue in propertiesArg.Values)
                    {
            ...
            foreach (AttributeData attribute in property.GetAttributes())
            {
                ...
                switch (attributeName)
                {
                    ...
                    case "PrimaryKey":
                        ...
                        foreach (KeyValuePair<string, TypedConstant> namedArg in attribute.NamedArguments)
                        {
                            // Check for both AutoGenerate (PrimaryKeyAttribute) and AutoIncrement (legacy/alternate naming)
