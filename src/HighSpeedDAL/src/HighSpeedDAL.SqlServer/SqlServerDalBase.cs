    ...
    where TConnection : DatabaseConnectionBase
{
    ...
    {
        var result = new BulkInsertResult<TEntity>
        {
        ...
        var remainingEntities = new List<TEntity>(entities);
        ...
        while (remainingEntities.Count > 0 && retryCount < maxRetries)
        {
            ...
            catch (SqlException ex) when (ex.Number == 2601 || ex.Number == 2627)
            {
                ...
                var (indexName, duplicateValue) = ParseDuplicateKeyError(ex.Message);
                ...
                var matchingEntities = duplicateKeyMatcher(indexName, duplicateValue, remainingEntities);
    ...
    private static (string indexName, string duplicateValue) ParseDuplicateKeyError(string message)
    {
        ...
        var indexMatch = Regex.Match(message, @"(?:index|constraint)\s+'([^']+)'", RegexOptions.IgnoreCase);
        ...
        var valueMatch = Regex.Match(message, @"duplicate key value is \(([^)]+)\)", RegexOptions.IgnoreCase);
