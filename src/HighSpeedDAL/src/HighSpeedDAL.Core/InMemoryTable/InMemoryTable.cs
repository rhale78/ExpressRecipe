    ...
    public sealed class InMemoryTable<TEntity> : IDisposable where TEntity : class, new()
    {
        ...
        public async Task<int> TriggerFlushAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                List<TEntity> entitiesToFlush = _rows.Values
        ...
        private InMemoryTable(ILoggerFactory? loggerFactory, ILogger logger, InMemoryTableAttribute? config = null, string? tableName = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(_config.MemoryMappedFileName))
            {
                ...
                ILogger<MemoryMappedFileStore<TEntity>> storeLogger = _loggerFactory.CreateLogger<MemoryMappedFileStore<TEntity>>();
                ILogger<MemoryMappedSynchronizer> syncLogger = _loggerFactory.CreateLogger<MemoryMappedSynchronizer>();
                ...
                {
                    List<TEntity> loadedRows = _memoryMappedStore.LoadAsync().GetAwaiter().GetResult();
                    foreach (TEntity entity in loadedRows)
                    {
        ...
        public Task<int> BulkInsertAsync(IEnumerable<TEntity> entities, CancellationToken cancellationToken = default)
        {
            ...
            int count = 0;
            List<TEntity> entityList = entities.ToList();  // Materialize to get count and allow batch operations
        ...
        public async Task<TEntity?> GetByPropertyAsync(string propertyName, object? value, CancellationToken cancellationToken = default)
        {
            ...
            if (cache.TryGetValue(cacheKey, out List<TEntity>? entities) && entities.Count > 0)
            {
        ...
        public async Task<List<TEntity>> GetByPropertyAsync(string propertyName, object? value, bool returnMultiple, CancellationToken cancellationToken = default)
        {
            ...
            string cacheKey = GetCacheKey(value);
            if (cache.TryGetValue(cacheKey, out List<TEntity>? entities))
            {
        ...
        private Dictionary<string, List<TEntity>>? EnsurePropertyCacheBuilt(string propertyName)
        {
            ...
            if (_propertyValueCache.TryGetValue(propertyName, out Dictionary<string, List<TEntity>>? existingCache))
            {
            ...
            lock (_cacheLock)
            {
                ...
                ColumnDefinition? column = _schema.Columns.FirstOrDefault(c =>
                ...
                Dictionary<string, List<TEntity>> cache = new Dictionary<string, List<TEntity>>(StringComparer.OrdinalIgnoreCase);
        ...
        public TEntity? GetById(object id)
        {
            ...
            return _rows.TryGetValue(id, out InMemoryRow? row) && row.State != RowState.Deleted ? row.ToEntity<TEntity>() : null;
        }
        ...
        public Task<int> UpdateAsync(TEntity entity, CancellationToken cancellationToken = default)
        {
            ...
            Dictionary<string, IndexKey> oldKeys = [];
            foreach (InMemoryIndex index in _schema.Indexes)
            {
        ...
        public async Task FlushToMemoryMappedFileAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                List<TEntity> entities = _rows.Values
                    .Where(r => r.State != RowState.Deleted)
                    .Select(r => r.ToEntity<TEntity>())
                    .ToList();
