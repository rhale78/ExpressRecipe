...
namespace HighSpeedDAL.Core.InMemoryTable
{
    ...
    public sealed class MemoryMappedFileStore<T> : IDisposable where T : class
    {
        ...
        private void CreateNewFile()
        {
            ...
            using (FileStream fileStream = new FileStream(_filePath, FileMode.Create, FileAccess.ReadWrite, FileShare.None))
            {
                fileStream.SetLength(_fileSizeBytes);
            }

            // Open as memory-mapped file
            using FileStream fileStream2 = new FileStream(_filePath, FileMode.Open, FileAccess.ReadWrite, FileShare.ReadWrite);
        ...
        private void ValidateSchema()
        {
            ...
            using MemoryMappedViewAccessor accessor = _mmf.CreateViewAccessor(0, HeaderSize, MemoryMappedFileAccess.Read);
        ...
        private void WriteHeader(int rowCount, long dataOffset)
        {
            ...
            using MemoryMappedViewAccessor accessor = _mmf.CreateViewAccessor(0, HeaderSize, MemoryMappedFileAccess.Write);
        ...
        public async Task<List<T>> LoadAsync(CancellationToken cancellationToken = default)
        {
            ...
            using IDisposable readLock = await _synchronizer.AcquireReadLockAsync(cancellationToken);
            ...
            // Read data section
            using MemoryMappedViewAccessor dataAccessor = _mmf.CreateViewAccessor(dataOffset, dataLength, MemoryMappedFileAccess.Read);
            ...
            // Deserialize
            List<T> rows = MessagePackSerializer.Deserialize<List<T>>(data.AsMemory(0, actualDataLength));
        ...
        public async Task SaveAsync(List<T> rows, CancellationToken cancellationToken = default)
        {
            ...
            using IDisposable writeLock = await _synchronizer.AcquireWriteLockAsync(cancellationToken);
            ...
            // Write data section
            using MemoryMappedViewAccessor dataAccessor = _mmf.CreateViewAccessor(MinDataOffset, data.Length, MemoryMappedFileAccess.Write);
