...
public sealed class TableSwapFlushStrategy<TEntity> : IFlushStrategy<TEntity> where TEntity : class, new()
{
    ...
    public async Task<int> FlushAsync(List<TEntity> entities, CancellationToken cancellationToken = default)
    {
        ...
        if (_config.DatabaseProvider.Equals("SqlServer", StringComparison.OrdinalIgnoreCase))
        {
            return await FlushSqlServerAsync(entities, cancellationToken);
        }
        else if (_config.DatabaseProvider.Equals("Sqlite", StringComparison.OrdinalIgnoreCase))
        {
            return await FlushSqliteAsync(entities, cancellationToken);
        }
        else
        {
    ...
    private async Task<int> FlushSqlServerAsync(List<TEntity> entities, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            using var dropCmd = new SqlCommand(dropSql, connection, transaction);
            ...
            using var renameCmd = new SqlCommand(renameSql, connection, transaction);
