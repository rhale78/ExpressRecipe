...
public class MockDatabaseConnection : IDbConnection
{
    ...
    private readonly List<(string sql, Dictionary<string, object?> parameters)> _executedQueries = new();
    ...
    public async Task<object?> ExecuteScalarAsync(string sql, Dictionary<string, object?>? parameters = null, CancellationToken cancellationToken = default)
    {
        ...
        if (sql.Contains("SCOPE_IDENTITY", StringComparison.OrdinalIgnoreCase) || 
            sql.Contains("last_insert_rowid", StringComparison.OrdinalIgnoreCase))
        {
            return _nextAutoIncrementId - 1;
        }
    ...
    public async Task<IDataReader?> ExecuteReaderAsync(string sql, Dictionary<string, object?>? parameters = null, CancellationToken cancellationToken = default)
    {
        ...
        if (match.Success)
        {
            ...
            try
            {
                var rows = _dataStore.Select(tableName);
                return new MockDataReader(rows);
            }
            catch { }
        }

        return new MockDataReader(new List<Dictionary<string, object?>>());
    }
    ...
    {
        var match = System.Text.RegularExpressions.Regex.Match(sql, @"INTO\s+\[?(\w+)\]?", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
        if (!match.Success)
            return 0;

        string tableName = match.Groups[1].Value;
        var row = new Dictionary<string, object?>(parameters, StringComparer.OrdinalIgnoreCase);
    ...
    private int ExecuteUpdate(string sql, Dictionary<string, object?> parameters)
    {
        ...
        var whereMatch = System.Text.RegularExpressions.Regex.Match(sql, @"WHERE\s+(\w+)\s*=\s*@(\w+)", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
...
public class MockDbCommand : IDbCommand
{
    ...
    public IDataReader ExecuteReader() => new MockDataReader(new());
    public IDataReader ExecuteReader(CommandBehavior behavior) => new MockDataReader(new());
...
public class MockDataReader : IDataReader
{
    ...
    public int GetValues(object?[] values)
    {
        if (_currentIndex < 0 || _currentIndex >= _rows.Count)
            return 0;
        var kvps = _rows[_currentIndex].ToArray();
