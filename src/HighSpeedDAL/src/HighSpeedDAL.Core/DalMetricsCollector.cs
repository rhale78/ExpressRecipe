...
namespace HighSpeedDAL.Core
{
    ...
    public class DalMetricsCollector : IDisposable
    {
        ...
        private readonly Dictionary<string, Counter<long>> _tableOperationCounters = [];
        // Key: tableName, Value: hits counter
        private readonly Dictionary<string, Counter<long>> _tableCacheHitCounters = [];
        // Key: tableName, Value: misses counter
        private readonly Dictionary<string, Counter<long>> _tableCacheMissCounters = [];
        // Key: tableName, Value: item count gauge
        private readonly Dictionary<string, ObservableGauge<int>> _tableItemCountGauges = [];
        // Key: tableName, Value: cache size gauge
        private readonly Dictionary<string, ObservableGauge<int>> _tableCacheSizeGauges = [];
        // Store current counts
        private readonly Dictionary<string, int> _itemCounts = [];
        private readonly Dictionary<string, int> _cacheSizes = [];
        ...
        private readonly Dictionary<string, Histogram<long>> _durationHistograms = [];
        // Key: tableName_operationType, Value: per-item duration histogram (for batch ops)
        private readonly Dictionary<string, Histogram<double>> _perItemDurationHistograms = [];
        // Key: tableName_operationType, Value: items-per-second histogram (for batch ops)
        private readonly Dictionary<string, Histogram<long>> _itemsPerSecondHistograms = [];
        ...
        public void RecordOperation(string tableName, string operationType, long count = 1, long durationMs = 0)
        {
            ...
            {
                KeyValuePair<string, object?>[] totalTags = new[] {
        ...
        public void RecordCacheHit(string tableName)
        {
            ...
            {
                KeyValuePair<string, object?>[] tags = new[] { new KeyValuePair<string, object?>("table", (object)tableName) };
